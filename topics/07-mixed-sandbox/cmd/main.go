...
func main() {
	...
	i := 5
	var iInt1 interface{} = i
	var iInt2 interface{} = &i
	fmt.Println(iInt1, iInt2)

	if v, ok := iInt1.(*int); ok && v != nil {
		fmt.Println("внутри интерфейса iInt1 не nil-указатель")
	}
	if v, ok := iInt2.(*int); ok && v != nil {
		fmt.Println("внутри интерфейса iInt2 не nil-указатель", v, *v)
	}
...
func Pop(h *[]int) int {
	...
	x := sl[n-1]
	*h = sl[2 : n-1]
	fmt.Println(len(*h), cap(*h), fmt.Sprintf("%p", *h))
	fmt.Println(len(sl), cap(sl), fmt.Sprintf("%p", sl))
	sl[0] = 123
	return x
}
...
func popFromSlice() {
	...
	fmt.Println(Pop(&h)) // 9
	fmt.Println(h)       // [123 2 3 4 5 6 7 8]
}
