...
Маршруты:
  ...
  НО планирование — VRP (NP-трудная); расчёт last-mile на 1000+ точек = секунды-минуты CPU
  Консолидация: 20M заказов / 15K маршрутов ≈ ~1300 заказов на маршрут
  → характерно для посылочной last-mile; планировщик БАТЧИТ заказы по гео+окну
...
*Почему stateless + Redis routing:* 10K соединений не помещаются на одну ноду; маршрутизация `driver→node` в Redis делает ноды взаимозаменяемыми — как connection-слой в [04. Chat](./04-chat-messaging.md). Протокол — [networking / WebSocket](../../08-networking-and-api/protocols/05-websocket.md).
...
Для этапа в статусе READY:
  ...
  Выбор лучшего по стоимости/ETA → атомарно занять:
    SET driver:{id}:assignment {route_id} NX EX ...   -- нет двойного назначения
  Пуш назначения водителю через Mobile Gateway.

Логист может override автоматическое назначение через Web UI.
```

Геопоиск кандидатов и `NX`-лок против двойного назначения — детально в [06. Uber](./06-uber-ride-sharing.md).
...
`order.received` → Planner батчит заказы по гео+окну → строит этапы A→X→Y→B, присваивает типы, VRP-консолидирует в маршруты → создаёт `routes` + `order_legs`, заказ → PLANNED.
*Итог:* заказ разбит на связанные этапы; географически близкие заказы консолидированы в общие грузоперевозки (~1300/маршрут).
