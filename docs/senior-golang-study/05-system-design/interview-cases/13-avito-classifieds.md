...
**Почему `filter`, а не `must` для атрибутов:** clause в `filter` не участвует в скоринге, поэтому дешевле, и вдобавок **может кешироваться** (query cache на уровне сегментов). Оговорка «может» существенна: ES кеширует не всё подряд, а по эвристике — часто повторяющиеся условия на достаточно крупных сегментах. Точные `term`-фильтры вроде `city_id` или `gearbox` кешируются хорошо; диапазоны по числам высокой кардинальности (`price`, `mileage`) — обычно нет. Так что «Москва + Автомобили + Автомат» действительно ускорится, а вот «цена от 1 000 001 до 1 999 999» — нет. `must` нужен только для текстового скоринга.

**Пагинация:** не `from/offset` глубоко (деградирует на больших страницах — deep pagination), а `search_after` — cursor-based.

Но здесь важна деталь, которую легко упустить: **курсор обязан состоять ровно из тех полей, по которым идёт сортировка.** Если сортировка объявлена как `[boost_score, _score]`, а курсор строится по `[boost_score, listing_id]`, пагинация просто не работает — ES ждёт значения по своим sort-полям.

```
Проблема глубже: _score в качестве sort-поля для search_after ненадёжен.
Он вещественный, пересчитывается при изменении статистики индекса
и не гарантирует уникальности — страницы поедут при refresh.

Рабочий вариант — сортировать по стабильным полям и завершать
уникальным tie-breaker:

  "sort": [
    { "boost_score": "desc" },
    { "listing_id": "desc" }        ← уникальный tie-breaker
  ]
  "search_after": [1.4, 12345]

Если текстовая релевантность нужна в сортировке, её включают
в boost_score через function_score ЗАРАНЕЕ, а не сортируют по _score.
```
...
Решение — батчинг через Redis:
  ...
  Async flush каждые 30-60 сек — слив дельты должен быть АТОМАРНЫМ:
    ...
    отдельным консьюмером, где offset коммитится после записи.
...
| Курсор пагинации | Sort по `boost_score` + уникальный tie-breaker | Sort по `_score` | `_score` нестабилен между refresh — страницы «едут» |
...
>
> Поиск — Elasticsearch. Денормализую атрибуты в типизированные поля документа: фасетные фильтры + facet counts + морфология + гео. Атрибуты кладу в `filter`-clause — без скоринга и с кешированием точных термов.
>
...
>
> Синхронизация PostgreSQL → ES через Outbox + Kafka + Indexer, чтобы не было рассинхрона. Пагинация — search_after по boost_score с уникальным tie-breaker: по `_score` курсор строить нельзя, он нестабилен между refresh.
>
