...
*Почему object storage:* 150 PB активных данных; content-addressing даёт дедуп и иммутабельность чанков. Перед S3 — [CDN](../../08-networking-and-api/request-lifecycle/04-cdn-load-balancer-reverse-proxy.md) для частых скачиваний.
...
File watcher → пересчёт хешей → POST только изменённых чанков → Metadata обновляет версию → `file.changed` в Kafka → WebSocket-пуш онлайн-устройствам → те pull-ят новые чанки.
*Итог:* правка 1 байта в 1 GB файле = один новый чанк; изменение доезжает до устройств за ≤ 30 сек.
