...
stateDiagram-v2
    ...
    Open --> HalfOpen: прошёл timeout
    HalfOpen --> Closed: пробный запрос успешен
    HalfOpen --> Open: пробный запрос упал

    note right of Closed: запросы проходят<br/>считаем ошибки
    note right of Open: запросы блокируются<br/>немедленно
    note right of HalfOpen: пробуем N запросов
```
