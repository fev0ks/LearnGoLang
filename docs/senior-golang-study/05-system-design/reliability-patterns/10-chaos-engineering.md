...
Главная мысль: **ты не знаешь насколько надёжна твоя система пока не пробовал её сломать**. Тесты в CI проверяют логику. Chaos engineering проверяет **архитектуру** — как система ведёт себя когда что-то отказывает.
