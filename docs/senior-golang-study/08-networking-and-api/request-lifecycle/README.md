...
Если упростить путь до слоев, картина такая:
...
- [06 Response Return Caching And Browser Rendering](./06-response-return-caching-and-browser-rendering.md)
- [07 End To End Timeline And Where It Breaks](./07-end-to-end-timeline-and-where-it-breaks.md)

Как читать:
- сначала пройти browser и DNS;
- потом соединение и HTTP;
- затем edge и backend;
- в конце посмотреть full timeline и точки деградации.

Что важно уметь объяснить после чтения:
- почему один запрос не равен "просто сходили по HTTP";
- где участвуют DNS cache, browser cache, CDN cache, OS cache, connection reuse;
- где тратится latency;
- почему проблема может быть не в приложении, а в DNS, TLS, LB, proxy или browser.
