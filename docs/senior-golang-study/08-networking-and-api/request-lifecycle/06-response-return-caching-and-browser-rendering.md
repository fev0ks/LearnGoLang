...
- `max-age` действует и на browser, и на CDN; `s-maxage` — только на shared caches (CDN) и переопределяет для них `max-age`; `private` разрешает только browser cache. Это позволяет держать разные политики: например, CDN кэширует 60 секунд, а браузер всегда ревалидирует.

**2. `no-cache` vs `no-store`?**

- `no-cache` кэшировать **разрешает**, но требует ревалидации перед использованием (условный запрос). `no-store` запрещает сохранять ответ вообще — для паролей и платёжных данных. Частая ошибка — ставить `no-cache` там, где нужен `no-store`.

**3. Как работает ETag?**

- Сервер отдаёт fingerprint версии ресурса; когда кэш устарел, клиент шлёт `If-None-Match`, и при совпадении сервер отвечает `304 Not Modified` без тела — экономия трафика. Надёжнее `Last-Modified` (секундная гранулярность, clock skew). `stale-while-revalidate` дополняет: отдать stale мгновенно, обновить в фоне.

**4. Чем опасен `Vary: Cookie`?**

- Каждое уникальное значение Cookie — отдельная запись в CDN-кэше; при миллионах пользователей это фактически «не кэшировать»: вся нагрузка уходит в origin. `Vary` — только на заголовки, реально меняющие контент (`Accept-Encoding`).

**5. Как инвалидировать CDN-кэш и на что backend влияет в Core Web Vitals?**

- Для статики надёжнее URL versioning (hash в имени файла — новый URL = новый контент); для остального — purge API или surrogate keys. Из CWV (LCP, INP, CLS) backend влияет прежде всего на LCP через TTFB: медленный origin не спасти оптимизацией фронтенда.
