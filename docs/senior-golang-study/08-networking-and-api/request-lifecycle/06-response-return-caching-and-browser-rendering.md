# Ответ, кэширование и отрисовка в браузере
...
```
Умеренно: если контент действительно различается по языку — нормально. Но `Accept-Language` + `Cookie` вместе дают огромную фрагментацию кэша.
...
**1. Чем `max-age`, `s-maxage` и `private` отличаются?**

- `max-age` действует на все кэши по пути: и на браузер, и на общие кэши вроде CDN.
- `s-maxage` относится только к общим кэшам и переопределяет для них `max-age`.
- `private` разрешает хранить ответ только браузеру: общие кэши обязаны его пропустить.
- Практическая связка — общий кэш держит ответ минуту, а браузеру предписывается перепроверять каждый раз; так обновление доходит быстро, а нагрузка на источник остаётся низкой.

**2. Чем `no-cache` отличается от `no-store`?**

- `no-cache` хранить разрешает, но требует перепроверки перед каждым использованием.
- `no-store` запрещает сохранять ответ вообще — это режим для паролей, платёжных данных и персональных документов.
- Частая ошибка — поставить `no-cache` там, где нужен `no-store`: ответ ляжет на диск и переживёт закрытие вкладки.
- Проверка понимания — `no-cache` не означает «не кэшировать»: он разрешает хранить, но обязывает перепроверять.

**3. Как работает ETag?**

- Сервер отдаёт отпечаток версии ресурса, клиент сохраняет его вместе с ответом.
- Когда срок годности истёк, клиент отправляет условный запрос с `If-None-Match`.
- При совпадении сервер отвечает `304 Not Modified` без тела: тратится время на запрос, но не на передачу содержимого.
- Надёжнее `Last-Modified`, у которого точность одна секунда и который страдает от расхождения часов.
- Дополняющая директива — `stale-while-revalidate`: устаревший ответ отдаётся сразу, а обновление происходит в фоне.

**4. Чем опасен `Vary: Cookie`?**

- Каждое уникальное значение заголовка порождает отдельную запись в кэше.
- При миллионах пользователей это означает практически полное отсутствие кэширования: почти каждый запрос уходит в источник.
- Правило — перечислять в `Vary` только заголовки, которые действительно меняют содержимое, например кодировку сжатия.
- Если персонализация нужна, её выносят из кэшируемого ответа: общий каркас кэшируется, персональная часть догружается отдельно.

**5. Как инвалидировать кэш CDN и на что backend влияет в Core Web Vitals?**

- Для статики надёжнее всего версионирование адреса: хеш в имени файла делает новый ресурс новым URL, старый истечёт сам.
- Для остального — сброс через API провайдера или пометки-ключи, но это операция с задержкой и лимитами.
- Из метрик Core Web Vitals backend влияет прежде всего на LCP, и влияет через время до первого байта.
- Вывод для спора с фронтендом — медленный источник не компенсируется оптимизацией на клиенте: сначала TTFB, потом критический путь отрисовки.
