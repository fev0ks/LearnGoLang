# CDN, балансировщик, обратный прокси
...
**1. Чем балансировщик уровня L4 отличается от L7?**

- L4 работает с потоками TCP и UDP и не видит HTTP: минимальные накладные расходы, маршрутизация только по адресу и порту.
- L7 разбирает HTTP: маршрутизация по пути, хосту и заголовкам, терминация TLS, аутентификация, ограничение скорости.
- Цена L7 — разбор протокола на каждый запрос и больше ресурсов.
- Типовая схема — L4 снаружи принимает соединения, L7 внутри принимает решения по содержимому запроса.

**2. Как CDN решает, кэшировать ли ответ?**

- Основа — заголовок `Cache-Control`: `s-maxage` управляет только общим кэшем, `private` оставляет кэширование браузеру, `no-store` запрещает везде.
- Персонализация — `Set-Cookie` в ответе и `Vary: Cookie` практически выключают кэш на стороне CDN.
- Инвалидация — по TTL, явным сбросом через API или, что надёжнее, версионированием адреса ресурса.
- Практический вывод — статику с хешем в имени можно кэшировать очень долго, HTML почти всегда требует перепроверки.

**3. Активные или пассивные проверки здоровья?**

- Активные — балансировщик сам опрашивает эндпоинт по интервалу и убирает инстанс до того, как в него попадёт пользовательский трафик.
- Пассивные — решение принимается по реальным ответам: серия ошибок исключает инстанс из ротации.
- Компромисс — пассивные реагируют быстрее, но платят за это запросами реальных пользователей.
- Практика — активные обязательны, пассивные дополняют; при выкатке нужен вывод из ротации с ожиданием текущих запросов дольше самого долгого из них.

**4. Почему таймаут прокси должен быть больше таймаута приложения?**

- Если прокси сдаётся раньше, клиент получает 504, а backend продолжает считать ответ, который уже никому не нужен.
- Ресурсы при этом продолжают тратиться: соединение с базой, память, процессор.
- Правило — таймауты выстраиваются по убыванию снаружи внутрь, чтобы первым сдавался самый внутренний уровень.
- Дополнительно — у самого приложения должен быть свой предел, иначе отмена со стороны прокси ни на что не влияет.
