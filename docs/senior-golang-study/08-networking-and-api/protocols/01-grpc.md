...
gRPC не «использует HTTP/2 как транспорт», а спроектирован вокруг его механики (подробный разбор HTTP/2 — фреймы, streams, HPACK, flow control — в [14-http2-and-http3.md](./14-http2-and-http3.md)):
...
- **статус в trailers** — `grpc-status`/`grpc-message` едут в заголовках *после* тела, потому что при стриминге статус известен только в конце. Именно из-за trailers браузер не может говорить на «чистом» gRPC (fetch не даёт доступ к trailers) — отсюда grpc-web;
