...
**1. Как защитить webhook endpoint?**

- Три уровня:
1. **HMAC подпись**: провайдер подписывает payload своим секретом → получатель верифицирует HMAC-SHA256, используя `hmac.Equal` (constant-time) — защита от timing attack.
...
**2. Почему webhook + at-least-once delivery?**

- Провайдер не знает, обработано ли сообщение, пока не получит 2xx. При network timeout, рестарте сервера, 5xx — он повторит. Это intentional: лучше доставить дважды, чем потерять. Ответственность получателя — сделать handler идемпотентным через проверку event ID.
