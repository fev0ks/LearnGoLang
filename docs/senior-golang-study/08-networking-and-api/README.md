...
- [UDP: датаграммы, границы сообщений и своя надёжность](./protocols/01-transport/02-udp.md) — что гарантируется и что нет, размер датаграммы и фрагментация, что приходится дописывать самому, зачем UDP понадобился QUIC, сокет в Go, тихие отказы
- [TLS: шифрование, доверие и стоимость рукопожатия](./protocols/01-transport/03-tls.md) — что согласуется в рукопожатии, cipher suite по частям, forward secrecy, цепочка сертификатов, записи, возобновление сессии, чтение ошибок TLS
