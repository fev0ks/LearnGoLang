...
Первые два пункта — про транспорт до клиента, а не про брокер, и порядок между ними именно такой: SSE проверяется раньше, потому что решает более узкую задачу более простыми средствами. Разбор самих транспортов — [SSE и реалтайм-протоколы](../08-networking-and-api/protocols/12-sse-and-realtime.md) и [WebSocket](../08-networking-and-api/protocols/05-websocket.md).
