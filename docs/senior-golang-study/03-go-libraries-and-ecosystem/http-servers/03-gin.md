...
**Реальность**: производительность роутера редко является узким местом. Если твой сервис обрабатывает 10k RPS на одном инстансе — разница между gin и chi не измеримая в production. Bottleneck обычно: IO (DB, сеть), сериализация больших payload, GC давление.
