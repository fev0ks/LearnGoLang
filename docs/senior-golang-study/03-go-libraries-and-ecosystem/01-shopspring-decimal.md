...
**1. Почему нельзя float64 для денег?**

- `float64` хранит числа в двоичной системе: десятичные дроби вроде `0.1`/`0.2` имеют ошибки представления, и накопление операций даёт расхождения в копейках — недопустимо для финансов.

**2. Как работает shopspring/decimal?**

- Число хранится как integer + exponent без потерь точности; арифметика и сравнение — методами объекта (`Add`/`Mul`/`Cmp`), не операторами Go. В JSON сериализуется строкой, в PostgreSQL хранится как `NUMERIC`.
