...
Представь форму комментариев. Пользователь пишет: **`<script>alert('hi')</script>`**. Если сервер сохранил это и отдаёт другим пользователям как HTML — каждый увидевший комментарий получит alert. Это безобидный пример. Заменить `alert` на код, отправляющий куки атакующему, — и сайт уже работает на него.
...
При корректном использовании `html/template` (данные через `{{.Field}}`, без конкатенации строк) XSS практически невозможен.
...
Запрещает браузеру угадывать Content-Type. Без этого: сервер вернул `Content-Type: text/plain`, но в данных HTML с `<script>` — старые браузеры могли "догадаться", что это HTML, и выполнить.
...
Защита от clickjacking — атаки, где злоумышленник встраивает сайт в iframe и обманом заставляет пользователя кликать на скрытые элементы.
...
Запрещает API, которые не используются.
...
Подробнее про cookie атрибуты: [11-security/authentication/02-sessions-and-session-security.md](../authentication/02-sessions-and-session-security.md)
...
**На стороне frontend (даже для backend-разработчика):**
