...
**Container escape** — выход из контейнера на хостовую машину. Атакующий, попавший в один контейнер (например, через уязвимость в твоём приложении), получает доступ ко всему серверу: другим контейнерам, секретам ноды, в Kubernetes-кластере — потенциально ко всему control plane.
...
**Supply chain — отравление зависимостей.** Берёшь `FROM ubuntu:latest` или `FROM node:20`. Кто гарантирует, что в этом образе нет бэкдора? Реальные случаи: в 2018 на Docker Hub нашли 17 образов с встроенным crypto-miner — их скачали 5 миллионов раз. Атакующий публикует "полезный" образ, разработчики его используют, шахтёр майнит на их кластерах.

**Typosquatting** — образ с похожим именем. Хочешь `nginx`, написал `ngnix` — получил вредоносный образ от атакующего. То же с npm/pip/Go modules внутри образа.

**Уязвимые пакеты.** Базовый образ был собран год назад. С тех пор в OpenSSL нашли 5 critical CVE. Твой контейнер их унаследовал. Атакующий через сетевой запрос эксплуатирует старую libcurl и получает RCE (remote code execution) внутри контейнера — а дальше пытается escape.
...
**hostPath/bind mount хостовых директорий.** `-v /:/host` — монтируешь корень хоста в контейнер. Теперь приложение в контейнере может читать `/etc/shadow`, `/root/.ssh/`, секреты других контейнеров. Менее очевидный случай — `-v /var/lib/kubelet:/data` в Kubernetes: даёт доступ к секретам всех подов на ноде.
