...
| создать недостающие объекты | контроллеры |
...
действие контроллера: создать ещё 1 Pod
```
...
flowchart LR
    ...
    Controller --> Action["Создать недостающий Pod"]
...
| `kube-controller-manager` | процесс с набором циклов управления | замечает расхождение между желаемым и наблюдаемым состоянием и создаёт необходимые изменения |
| `kube-scheduler` | процесс-планировщик | выбирает `worker node` для нового Pod |
| `cloud-controller-manager` | необязательный процесс интеграции с облаком | связывает объекты Kubernetes с виртуальными машинами, маршрутами и балансировщиками провайдера |
...
2. контроллеры создают ReplicaSet и недостающий объект Pod
3. kube-scheduler выбирает для Pod worker node
4. kubelet выбранного узла запускает контейнеры
```
...
Важно: ReplicaSet controller **создаёт объект Pod через Kubernetes API**, но не запускает процесс контейнера на машине. После создания объект замечает планировщик, а до реального запуска цепочка доходит через `kubelet`.

Примеры других контроллеров:
...
| Node controller | объекты `Node` и сигналы доступности | отмечает недоступные узлы и участвует в обработке их отказа |
...
Контроллеры не выполняют всю операцию одной транзакцией. Deployment controller создаёт или настраивает `ReplicaSet`, ReplicaSet controller создаёт Pod, планировщик назначает узел, а `kubelet` запускает контейнеры. Поэтому результат появляется через несколько независимых циклов согласования.
...
`kubelet` работает на каждом узле и отвечает за назначенные **этому узлу** Pod.
...
`kubelet` не принимает решение, на каком узле должен работать Pod. Это уже записывает планировщик. `kubelet` также не поддерживает число реплик `Deployment`: за это отвечают контроллеры.
...
- обнаружении и удалении облачных виртуальных машин;
- создании внешнего балансировщика для `Service`;
- настройке маршрутов;
- заполнении облачных сведений об узлах.

В локальном или bare-metal кластере этого процесса может не быть. Тогда соответствующие задачи решают другие контроллеры и внешние инструменты.
...
| контроллер Ingress или Gateway | читает объекты с правилами маршрутизации внешнего HTTP-трафика (`Ingress`, Gateway API) и настраивает соответствующий прокси или балансировщик. Без такого контроллера объект `Ingress` принимается API, но ни на что не влияет |
...
flowchart TB
    ...
    Controllers["3. Контроллеры<br/>создают ReplicaSet и Pod"] -->
...
4. Контроллер Deployment замечает новый объект и создаёт `ReplicaSet`.
5. Контроллер ReplicaSet создаёт три объекта Pod.
...
11. Контроллер EndpointSlice добавляет адрес готового Pod, и тот может начать получать трафик через `Service`.
...
- **Автомасштабирование**: HPA может менять число реплик по метрикам.

Kubernetes не обязательно «перезапускает Pod». `kubelet` может перезапустить отдельный контейнер внутри существующего Pod, а контроллер создаёт новый Pod, когда нужна замена всей единицы запуска.
...
Планировщик выбирает узел для нового Pod. Контроллеры создают или изменяют объекты, чтобы приблизить наблюдаемое состояние к желаемому. `kubelet` на конкретном узле запускает назначенные ему Pod и отправляет их состояние в API.
...
API-сервер проверяет и сохраняет объект. Контроллеры создают зависимые объекты, планировщик выбирает узел, `kubelet` замечает назначенный Pod, а среда выполнения запускает контейнеры. Каждый компонент выполняет свой этап через наблюдение Kubernetes API.
