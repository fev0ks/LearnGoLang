...
- [Когда использовать `ports`, а когда нет](#когда-использовать-ports-а-когда-нет)
...
Нужен `ports`:
- API, доступный с хоста;
- UI сервисы вроде Grafana;
- debug endpoints, открываемые браузером или `curl`.
