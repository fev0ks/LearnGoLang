...
Граница с соседним материалом: здесь — как сокеты выглядят из приложения и как их настраивать и диагностировать в Linux; механика самого протокола (подтверждения, повторные передачи, окна, управление перегрузкой) разобрана в [TCP: надёжность, окна и перегрузка](../../08-networking-and-api/protocols/01-tcp-mechanics.md).
