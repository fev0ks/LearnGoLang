...
**1. Как работает виртуальная память?**

- Каждый процесс имеет изолированное виртуальное адресное пространство; MMU транслирует адреса через page tables (4 уровня на x86-64) в физические. TLB кэширует трансляции; context switch его инвалидирует — часть цены переключения.

**2. Чем minor page fault отличается от major?**

- Minor — страница уже в RAM, нужно лишь добавить запись в page table (быстро); major — чтение с диска (медленно). Copy-on-write: после `fork()` обе копии делят физические страницы до первой записи.

**3. Как Go использует mmap и что с OOM?**

- Go runtime выделяет heap-арены anonymous mmap-ом; из-за overcommit `mmap`/`malloc` всегда успешны, физическая память подключается при первой записи — поэтому VmSize > VmRSS. OOM killer выбирает жертву по oom_score (~пропорционален RSS); `GOMEMLIMIT` заставляет GC работать агрессивнее и снижает риск OOMKilled.

**4. Что такое page cache?**

- Ядро кэширует файловый I/O в свободной RAM; запись сначала попадает в dirty pages, `fsync` принудительно сбрасывает их на диск — на этом строится durability БД.
