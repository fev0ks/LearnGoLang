...
**2. PID (Process ID).**
Уникальный идентификатор. В Linux — целое число до 2^22 (4 миллиона) обычно. Первый процесс — `init` (PID 1, либо systemd, либо в контейнере — само приложение).
...
В Kubernetes / Docker контейнер — это namespace + cgroup + процесс. Если Go-сервис в контейнере запущен как PID 1 — он отвечает за reap'инг zombie-детей (например, после `exec.Command`).
...
**7. Знай /proc.**
`/proc/PID/status`, `/proc/PID/maps`, `/proc/PID/task/` полезны при отладке: показывают state, память, threads, open files без отладчика.
...
- [Переключение контекста и планирование CPU](./07-context-switching-and-scheduling.md) — следующий файл про планирование потоков ядром
