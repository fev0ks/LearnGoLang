...
Когда несколько ядер CPU работают с одной памятью, всё, что "известно" о порядке операций — **неправда**. CPU переупорядочивает инструкции для производительности. Компилятор переупорядочивает инструкции для производительности. И результат может удивить даже опытного разработчика.

Этот файл — про то как **на самом деле** работают atomic операции и memory ordering на уровне процессора. Из этого видно, что `sync/atomic` в Go — не магия, а тонкая работа с конкретными hardware-инструкциями. И увидишь почему в коде ниже возможен результат `(0, 0)`.
...
`(0, 0)` встречается **3000 раз из 10 миллионов**. Это не баг компилятора, не баг Go, не баг прикладного кода. Это **физика процессора**.
...
**Go runtime использует atomics с правильными fence'ами для своей платформы.** При использовании `sync.Mutex`, `sync/atomic`, channels Go скрывает разницу платформ — код будет корректным и на x86, и на ARM.

**Но ad-hoc concurrency без synchronisation** — например, "просто читаю переменную из другой goroutine без atomic" — даёт поведение, зависящее от модели памяти CPU. На x86 это часто **работает случайно**. На ARM — **скорее всего сломается**.

Поэтому золотое правило Go: **не допускать data race**. Race detector (`-race`) это ловит.
...
Три инструкции. Между чтением и записью другое ядро может изменить `counter` — результат запишет старое значение + 1, "потеряв" чужое изменение. Классический lost update.
...
Go memory model — это **контракт** между runtime'ом и программистом о том, какие гарантии есть для concurrent code.
