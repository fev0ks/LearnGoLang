...
- почему в `Prometheus UI` и `Grafana` вместо одной метрики видно “кучу строк”.

## Содержание

- [Самая короткая интуиция](#самая-короткая-интуиция)
- [Что реально видно в Prometheus UI](#что-реально-видно-в-prometheus-ui)
...
## Что реально видно в Prometheus UI
...
обычно видно много строк, потому что они различаются по labels:
- `job`
- `instance`
- `pod`
- `route`
...
Теперь это специально показывает imbalance между pod'ами.
...
2. определить, какой уровень нужен:
   - service
   - route
   - pod
   - status_code
