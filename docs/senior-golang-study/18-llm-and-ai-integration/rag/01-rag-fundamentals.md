...
- Она не видела твоих приватных доков
...
- Внутреннюю документацию твоей компании
...
Идея:
1. У тебя есть **внешний источник знаний** — документы, БД, веб
2. На запрос пользователя ты **находишь** релевантные куски в этом источнике
3. **Передаёшь** найденное в LLM как контекст
4. LLM **генерирует** ответ опираясь на этот контекст

```
User: "Какая политика возврата у нашей компании?"
                    │
                    ▼
            [ Search в БД политик ]
                    │
                    ▼
   Найдено: "Возврат возможен в течение 30 дней..."
                    │
                    ▼
   ┌─────────────────────────────────────────┐
   │ Контекст для LLM:                        │
   │ "Используй следующую информацию для      │
   │  ответа: [найденный текст]               │
   │  Вопрос пользователя: [оригинальный]"    │
   └─────────────────────────────────────────┘
                    │
                    ▼
                [  LLM  ]
                    │
                    ▼
   Ответ: "Согласно политике компании, возврат
           возможен в течение 30 дней с момента
           покупки..."
```
...
Три способа адаптировать LLM к твоей задаче. Часто путают.
...
2. **Кэширование запросов.** Часто задают похожие вопросы — кэшируй ответ.
