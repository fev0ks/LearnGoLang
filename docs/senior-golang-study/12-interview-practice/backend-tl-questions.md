...
- **Coverage gate и race detector** — порог покрытия и `go test -race` как обязательные шаги CI.
...
- **Масштабирование:** horizontal scaling и autoscaling (HPA по RPS/custom-метрикам), load balancing, async-обработка через очередь (отвечаем `202`, обрабатываем фоном), кэширование на всех уровнях (CDN/edge/app/DB), read replicas, sharding, connection pooling.
