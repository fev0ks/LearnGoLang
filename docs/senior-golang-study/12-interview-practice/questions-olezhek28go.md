...
Разбор тренажёра к ролику «Worker Pool в Go»: 20 вопросов с реальных Go-собеседований по конкурентности, рантайму, каналам, интерфейсам и HTTP. По каждому вопросу дан краткий ответ по существу, типичные ошибки и ссылки на разделы конспекта с глубоким разбором.
...
- Вернуть первый успешный ответ, не дожидаясь остальных.
...
- Горутины дёшевы (около 2–8 КБ стартового стека) и при I/O-блокировке не занимают поток ОС: на блокирующем syscall/сетевом ожидании рантайм через netpoller паркует горутину и освобождает `M` под другие. Поэтому 10 000 горутин, ждущих сеть, почти не стоят CPU.
- Пул воркеров нужен прежде всего чтобы не превысить лимит внешнего ресурса — коннекты к БД, квоты API, файловые дескрипторы. Но такое ограничение проще выразить семафором (`chan struct{}` или `errgroup.SetLimit`), чем полноценным пулом.
...
- `doWork` возвращает `*MyError(nil)`: значение-указатель нулевое, а вот тип задан — `*MyError`. При присваивании в `error` интерфейс получает пару `(*MyError, nil)`. Тип непустой, поэтому интерфейс не равен `nil`, и сравнение даёт `false`. Это и есть «typed nil».
...
- В обёртках над ошибками проверять конкретный указатель до возврата его как `error`.
