...
- Интерфейс из четырёх методов:
  - `Done() <-chan struct{}` — канал, который закрывается при отмене.
  - `Err() error` — после отмены возвращает `Canceled` или `DeadlineExceeded`.
...
- Практическая модель — трёх состояний достаточно:
  - runnable — лежит в очереди планировщика и ждёт P.
  - running — исполняется на M вместе с P.
  - waiting — запаркована на канале, мьютексе, таймере или сетевом ожидании.
...
- Это четыре гарантии транзакции:
  - Atomicity (атомарность) — транзакция применяется целиком или не применяется вовсе; при сбое `ROLLBACK` откатывает всё.
  - Consistency (согласованность) — после коммита данные не нарушают ограничения схемы и бизнес-инварианты.
  - Isolation (изолированность) — параллельные транзакции не портят результат друг друга; насколько именно, задаёт уровень изоляции.
  - Durability (долговечность) — после коммита изменения переживают перезапуск и обычный сбой; в PostgreSQL это обеспечивает WAL с fsync.
- Где проходит граница атомарности — по изменениям в одной БД. Внешний HTTP-вызов внутри транзакции откатить нельзя: запрос уже ушёл, а транзакция всё это время держит соединение из пула и блокировки строк.
...
- Три свойства распределённой системы, из которых одновременно достижимы не все:
  - Consistency — каждое чтение видит последнюю успешную запись либо получает ошибку. Строго это линеаризуемость, а не «данные валидны по ограничениям», как в ACID.
  - Availability — каждый запрос к живому узлу получает ответ без ошибки.
  - Partition tolerance — система продолжает работать при потере связи между узлами.
- Как читать теорему на практике — P не выбирают: сеть ненадёжна по определению, разрывы будут. Поэтому реальный выбор звучит как CP или AP именно в момент разрыва.
...
- Расшифровка BASE:
  ...
  - Soft state — состояние может меняться само, без пользовательской записи, из-за репликации и согласования.
  - Eventually consistent — если новые записи прекратятся, реплики со временем сойдутся к одному состоянию.
- Eventual consistency — узлы временно показывают разные значения, но при отсутствии новых изменений сходятся. На собесе полезно добавить, что «временно» — это конкретное измеримое окно, а не абстракция.
...
- Это две стратегии управления конкурентным доступом:
...
- Брокер — узел кластера, хранит партиции и обслуживает клиентов.
- Топик — логическая категория сообщений.
- Партиция — упорядоченный лог внутри топика, единица параллелизма и репликации.
- Продюсер и консьюмер — кто пишет и кто читает; консьюмеры объединяются в consumer group.
...
- At-most-once — не более одного раза: возможна потеря, дублей нет. Offset коммитят до обработки, продюсер шлёт с `acks=0`.
- At-least-once — не менее одного раза: потерь нет, возможны дубли. Offset коммитят после обработки; самый частый выбор, требует идемпотентной обработки.
- Exactly-once — ровно один раз: идемпотентный продюсер плюс транзакции.
- Чем регулируется — настройкой `acks`, политикой ретраев и моментом коммита offset.
...
- Поверх TCP — HTTP/1.1, HTTP/2, HTTPS/TLS, gRPC, SMTP, FTP, протоколы баз данных.
- Поверх UDP — DNS, DHCP, RTP и VoIP, QUIC вместе с HTTP/3 на его базе.
- Что определяет выбор — требование к надёжности против требования к латентности.
...
- Зачем — стандартный способ сообщить клиенту результат, чтобы он реагировал единообразно: ретрай, ошибка, редирект.
- Классов пять:
  - 1xx — информационные.
...
- Сначала уточнить термин — «поверх HTTP» покрывает три разные вещи: HTTP как транспорт сообщений, HTTP только как способ установить соединение и стили поверх обычных HTTP-запросов. На собесе полезно это развести, а не сыпать списком.
- API-стили и RPC:
  - REST — архитектурный стиль на семантике HTTP: методы, коды, кэширование.
  - gRPC — RPC поверх HTTP/2 с бинарным Protobuf; в браузере доступен как gRPC-Web через прокси.
  - GraphQL — обычно один endpoint и запросы через POST.
  - SOAP — XML-конверт, исторически поверх HTTP.
  - JSON-RPC и XML-RPC — простые RPC поверх POST.
- Realtime:
  - SSE — однонаправленный поток сервер → клиент прямо в теле HTTP-ответа; действительно работает поверх HTTP.
  - WebSocket — использует HTTP только для рукопожатия `Upgrade`, дальше это самостоятельный кадровый протокол поверх того же TCP, а не HTTP.
  - WebTransport — двусторонний обмен поверх HTTP/3.
- Интеграции и работа с данными:
  - Webhooks — обратные HTTP-вызовы от провайдера к вашему сервису.
  - WebDAV и надстройки над ним CalDAV/CardDAV — файлы, календари, контакты.
  - S3 API — объектное хранилище обычными HTTP-запросами.
- Неочевидные, но частые:
  - DoH, DNS over HTTPS (RFC 8484) — DNS-запросы внутри HTTP.
  - HLS и MPEG-DASH — видеостриминг обычными GET-запросами за сегментами плейлиста.
  - OAuth 2.0, OIDC и SAML — протоколы авторизации, построенные на HTTP-редиректах и запросах.
  - ACME — выпуск TLS-сертификатов, на нём работает Let's Encrypt.
...
- Ключевая мысль — по HTTP сервис обычно говорит только наружу, с клиентами. К своим зависимостям он ходит по совсем другим протоколам.
- Свои протоколы поверх TCP у хранилищ и брокеров:
  - PostgreSQL и MySQL — собственные wire-протоколы; драйвер `pgx` говорит именно на нём, а не по HTTP.
  - Redis — текстовый протокол RESP.
  - MongoDB — свой wire protocol.
  - Kafka — собственный бинарный протокол, RabbitMQ — AMQP; рядом NATS и MQTT.
  - Memcached — свой простой текстовый протокол.
- Классические сетевые сервисы поверх TCP — SSH, почтовые SMTP, IMAP и POP3, FTP, LDAP, BGP.
- Поверх UDP — DNS на 53 порту, DHCP, NTP, SNMP, syslog, RTP и RTCP для медиа, StatsD, WireGuard, а также QUIC (уже на нём сверху живёт HTTP/3).
- Ниже транспортного уровня — ICMP (`ping`, `traceroute`) идёт прямо поверх IP, без портов; ARP работает на канальном уровне.
...
- Что это — эталонная модель из семи уровней (ISO/IEC 7498-1). Правило чтения: чем ниже уровень, тем меньше он знает о смысле передаваемых данных.
  - 1. Физический — биты, сигнал в среде.
  - 2. Канальный — кадры, MAC, Ethernet; отсюда же берётся MTU.
  - 3. Сетевой — пакеты, IP и ICMP, маршрутизация между разными сетями.
  - 4. Транспортный — сегменты и датаграммы, TCP/UDP/QUIC, порты и гарантии доставки.
  - 5. Сеансовый — диалог: открыть, поддерживать, восстановить после обрыва.
  - 6. Представления — кодировка, сжатие, шифрование.
  - 7. Прикладной — смысл обмена: HTTP, gRPC, DNS, SMTP.
- Чем OSI отличается от TCP/IP — интернет работает не на OSI, а на стеке TCP/IP из четырёх уровней (RFC 1122), где три верхних уровня OSI схлопнуты в один. Отдельных реализаций сеансового уровня и уровня представления не существует: сессию HTTP держит сам через cookie, сжатие согласует через `Accept-Encoding`, формат — через `Content-Type`.
...
- L4, транспортный уровень — балансировщик работает с потоками TCP и UDP и не видит HTTP:
  - Маршрутизация только по IP, порту и протоколу.
  - Минимальные накладные расходы и очень высокая пропускная способность.
  - Решать по URL, заголовкам и cookie не умеет — он их просто не разбирает.
  - TLS обычно не терминирует: соединение проходит насквозь до бэкенда.
  - Примеры — AWS NLB, GCP Network LB, Linux IPVS.
- L7, прикладной уровень — балансировщик разбирает сам HTTP-запрос:
  - Маршрутизация по пути, хосту, методу, заголовкам и cookie: `/api/*` в один кластер, `/static/*` в другой.
  - Терминация TLS, а дальше к upstream либо повторное шифрование, либо plaintext по доверенной внутренней сети.
  - Умеет добавлять и снимать заголовки, делать аутентификацию, ограничение частоты запросов, ретраи и circuit breaker.
  - Примеры — Nginx, Envoy, HAProxy, AWS ALB, Kubernetes Ingress.
- Ключевое различие — единица работы: у L4 это соединение, у L7 отдельный запрос.
- Что из этого следует на эксплуатации — за L4 долгоживущее соединение (WebSocket, поток gRPC, мультиплексированное HTTP/2) закрепляется за одним сервером на всё время жизни. Добавление реплики тогда не разгружает текущую нагрузку: новый сервер получит только новые соединения, а старые продолжат идти туда, куда легли изначально. L7 распределяет каждый запрос, поэтому реплика начинает работать сразу.
...
- Handshake (TLS 1.3, упрощённо) — три шага:
  - Клиент шлёт `ClientHello`: поддерживаемые версии, наборы шифров, key share.
  - Сервер отвечает `ServerHello` вместе со своим сертификатом и параметрами.
  - Стороны согласуют общий сеансовый ключ через ECDHE (это даёт forward secrecy) и дальше шифруют симметрично.
- Что проверяется в сертификате — цепочка подписей до корневого CA из доверенного хранилища ОС или браузера, срок действия, отзыв (CRL/OCSP) и совпадение домена из SAN с запрошенным; сервер попутно доказывает владение приватным ключом.
...
- Снаружи — `nc -zv host port` или `telnet host port`, `nmap -p 443 host` для скана портов, `curl -v host:port`.
- Изнутри, какие порты слушаются и кем:
  - `ss -tlnp` — TCP-сокеты в состоянии LISTEN с PID и именем процесса; современная замена `netstat -tlnp`.
...
- `top`/`htop` — CPU и память в реальном времени.
- `ps aux` — снимок по всем процессам.
...
- Что это — три числа (за 1, 5 и 15 минут): среднее количество процессов, которые выполняются или ждут (runnable плюс непрерываемое ожидание, обычно диск).
- Интерпретировать надо относительно числа ядер:
  - la равен числу ядер — загрузка около 100%.
  - la больше числа ядер — образовалась очередь, система перегружена.
...
- Навигация и файлы — `ls`, `cd`, `cat`, `less`, `head`, `tail`, `touch`, `cp`, `mv`, `rm`, `chmod`, `chown`, `wc`, `diff`, `tar`, `unzip`.
- Поиск — `grep`, `find`, `history`.
- Процессы — `ps`, `top`/`htop`, `kill`, `jobs`, `bg`/`fg`.
...
- Deployment — управляет жизненным циклом stateless-подов через ReplicaSet: держит нужное число реплик, делает rolling-update и rollback, пересоздаёт упавшие поды.
- Service — стабильная точка доступа к набору подов: у подов эфемерные IP, а Service даёт постоянное имя и virtual IP, балансируя трафик по живым подам через label-selector.
- Типы Service:
...
- Просмотр — `kubectl get pods/deploy/svc -n ns`, `kubectl describe pod <p>` (события и причины), `kubectl get events`.
...
- Базовая команда — `kubectl logs <pod>`: отдаёт stdout и stderr контейнера.
- Полезные флаги:
  - `-f` — следить за логом в реальном времени.
  - `-c <container>` — конкретный контейнер в многоконтейнерном поде.
...
- Всё стоит на фичах ядра Linux:
  - namespaces — изоляция: PID, NET, MNT, UTS, IPC, USER дают контейнеру свой список процессов, сеть и файловую систему.
  - cgroups — лимиты на CPU, память и I/O.
  - union/overlay-фс — слои образа только для чтения плюс writable-слой контейнера.
...
- Multi-stage build — собирать в одном образе, в финальный копировать только готовый артефакт.
- Минимальная база — scratch для статического Go-бинаря, иначе distroless или alpine.
- Статическая сборка — `CGO_ENABLED=0` плюс `-ldflags="-s -w"`, чтобы убрать отладочную информацию.
...
- bridge — вариант по умолчанию: приватная виртуальная сеть на хосте, наружу через NAT; user-defined bridge вдобавок даёт DNS по имени контейнера.
- host — контейнер использует сеть хоста напрямую, без изоляции: быстрее, но возможны конфликты портов.
...
- Чего не делать — зашивать в образ и передавать через `ENV`/`ARG`: такие значения видны в истории слоёв и в `docker inspect`.
- Варианты, как правильно:
  - Docker secrets — в Swarm монтируются в `/run/secrets` и живут только в памяти.
  - Внешние менеджеры — Vault, AWS Secrets Manager, SOPS.
...
- Механизм — флаги `docker run`, под капотом это cgroups.
- Память — `--memory=512m`, при необходимости вместе с `--memory-swap`.
...
- Пять принципов ООП-дизайна:
  - Single Responsibility — у типа одна причина для изменения.
  - Open/Closed — открыт для расширения, закрыт для изменения.
  - Liskov Substitution — подтип можно подставить вместо базового типа, не сломав поведение.
...
- Модель распределения тестов по трём уровням:
  - Основание — много быстрых и дешёвых unit-тестов.
...
- Counter — монотонно растущий счётчик (число запросов, ошибок); смотрят через `rate()`.
...
- Базовые — `timestamp`, `level`, `message`, `service` или `component`.
- Корреляция — `trace_id` и `span_id` для связи с трейсингом, плюс `request_id`.
- Контекст запроса — `user_id`, `method`, `path`, `status`, `duration`.
...
- `DEBUG` — детальная отладка, в проде обычно выключен.
- `INFO` — нормальные события: старт сервиса, обработанный запрос.
- `WARN` — подозрительное, но не ошибка: ретрай, деградация, приближение к лимиту.
- `ERROR` — операция не удалась и нужен разбор.
- `FATAL`/`PANIC` — критично, процесс завершается.
- Как выбирают — уровень задаётся конфигом: в проде обычно `INFO` и выше, `DEBUG` включают точечно.
- Зачем это важно — от уровня зависит, что попадёт в алерты и сколько шума окажется в логах.
...
**Что такое O-нотация? Как считать сложность по времени и по памяти?**

- Что это — асимптотическая оценка того, как растёт число операций или объём памяти при увеличении размера входа `n`. Оценивается именно скорость роста, а не время в секундах: константы и младшие слагаемые отбрасываются, поэтому `3n² + 100n + 7` записывается как O(n²).
- Зачем — сравнивать алгоритмы независимо от железа, языка и компилятора и заранее понимать, что произойдёт при росте данных на порядок.
- Время и память считают отдельно:
  - Временная сложность — сколько операций выполняется.
  - Пространственная — сколько дополнительной памяти нужно сверх самого входа; сюда входит и глубина стека рекурсии.
  - Между ними обычно идёт размен: хеш-таблица опускает поиск пары с O(n²) до O(n), но просит O(n) дополнительной памяти.
- Как считать составной алгоритм — три правила:
  - Последовательные блоки — берётся максимум: O(n) следом за O(n log n) в сумме даёт O(n log n).
  - Вложенные циклы — сложности перемножаются: два вложенных прохода по `n` дают O(n²).
  - Рекурсия — мастер-теорема для `T(n) = a·T(n/b) + f(n)`. У Merge Sort a = 2, b = 2, f(n) = O(n), отсюда O(n log n).
- Какой случай оценивают — по умолчанию худший (верхняя граница, O-нотация): система должна выдерживать любой вход, включая враждебный. Рядом существуют лучший случай (Ω) и средний (Θ).
...
- O(1), константная — доступ по индексу в массиве или слайсе, чтение и запись в `map`, `push`/`pop` стека, вставка в голову связного списка.
- O(log n), логарифмическая — бинарный поиск по отсортированному слайсу (`sort.SearchInts`), поиск и вставка в сбалансированном дереве (B-tree, AVL, красно-чёрное), вставка и извлечение в бинарной куче, быстрое возведение в степень.
- O(√n) — проверка числа на простоту перебором делителей до корня и факторизация тем же перебором.
- O(n), линейная — линейный поиск, любой одиночный проход по коллекции, подсчёт частот через `map`, метод двух указателей, алгоритм Кадане для максимального подмассива.
- O(n log n), линейно-логарифмическая — Merge Sort, Heap Sort, Quick Sort в среднем случае, `slices.Sort` и `sort.Slice` в Go (внутри pdqsort начиная с 1.19), плюс типовой приём «отсортировать, а потом пройти один раз».
- O(n²), квадратичная — Bubble Sort, Insertion Sort, Selection Sort, перебор всех пар элементов, наивное сравнение каждого элемента с каждым.
- O(n³), кубическая — наивное умножение матриц, алгоритм Флойда — Уоршелла для кратчайших путей между всеми парами вершин.
...
- Что это — структура «ключ → значение» с доступом за амортизированную O(1) через хеш-функцию и бакеты; в Go это встроенный `map`.
- Назначение — быстрый поиск, вставка и удаление по ключу.
- Ограничения:
  - Нет упорядоченности, поэтому недоступны диапазонные запросы и сортированный обход.
...
- Что это — массив с автоматическим расширением ёмкости; в Go это слайс над backing-массивом.
- Сложность — доступ по индексу O(1), `append` в конец — амортизированная O(1).
- Ограничения:
