...
В отличие от разбираемого решения, где `cancel()` валит **всё** при первой ошибке, здесь:
...
**Исключение:** если ошибка означает "downstream полностью down" — да, cancel разумен. Но обычно это неизвестно из обычной error.
...
В комментарии к решению отмечено:
```
...
### E. Order — разбираемое решение тоже не сохраняет
