...
Секция с упором на устройство ОС: виртуальная память, страницы, сисколлы, процессы и потоки, OOM, плюс Go-runtime (`GOMAXPROCS`) и финальный код-ревью кэша. Ответы краткие, в формате «как сказать на собесе». Где есть профильный материал в `docs/senior-golang-study/`, дана ссылка на файл.
...
- Что это — фиксированный блок виртуального адресного пространства, по умолчанию 4 КБ. Это минимальная единица, которой оперирует система управления памятью.
...
- Что это — запрос пользовательского процесса к ядру на привилегированную операцию, которую он не вправе выполнить сам.
...
- Классические примитивы:
  - Mutex — взаимное исключение, в критической секции один.
  - Семафор — счётчик разрешений, пускает до N одновременно.
  - Condition variable — ожидание условия вместе с мьютексом.
  - Spinlock — активное ожидание в цикле; выгоден только для очень коротких секций и на многоядерной машине.
  - Read-write lock — много читателей либо один писатель.
...
- Что это — целочисленный хендл, который ядро выдаёт процессу на открытый ресурс: файл, сокет, pipe, устройство.
...
- Три потока, открытых у процесса по умолчанию:
  - stdin, дескриптор 0 — стандартный ввод.
  - stdout, дескриптор 1 — стандартный вывод.
  - stderr, дескриптор 2 — вывод ошибок.
...
- Каналы — pipes и named pipes (FIFO): однонаправленный поток байтов, просто и повсеместно.
- Сокеты, включая Unix-domain — единственный вариант, который одинаково работает и внутри машины, и по сети.
- Разделяемая память — самый быстрый способ (копирования нет вообще), но синхронизацию придётся строить самому поверх семафоров или futex.
- Очереди сообщений — обмен готовыми сообщениями с границами, а не потоком байтов.
...
- Короткий ответ — O(n).
- В среднем — доступ, вставка и удаление за O(1).
- Когда становится O(n) — если все ключи схлопнулись в один бакет и его цепочку overflow-бакетов, либо во время роста, когда приходится смотреть и старый, и новый массив.
- Почему на практике это не случается — рантайм рандомизирует seed хеша при создании map, поэтому подобрать набор ключей-коллизий извне нельзя.
- Зачем вообще эта рандомизация — она защищает от hash-flooding: атаки, где злоумышленник специально подбирает ключи, чтобы деградировать сервис до квадратичного поведения.
...
- Что это — протокол прикладного уровня, работающий по модели запрос-ответ: клиент шлёт запрос, сервер возвращает ответ.
- Ключевое свойство — stateless: сам протокол не хранит состояние между запросами, каждый запрос самодостаточен.
...
- HTTP/1.0, 1.1 и 2 — поверх TCP: он даёт надёжную упорядоченную доставку, на которую HTTP опирается.
...
- Стартовая строка — метод, цель запроса и версия: `GET /v1/items HTTP/1.1`.
- Заголовки — `Host`, `Content-Type`, `Authorization`, `Cookie` и прочие, по одному на строку.
...
- С точки зрения протокола cookie это просто заголовки — сервер ставит их через `Set-Cookie` в ответе, клиент возвращает через `Cookie` в запросе.
- Что из этого следует — механизм поддержания состояния (RFC 6265) реализован целиком в заголовках, а не в теле и не в URL.
- Почему это важно — сам HTTP при этом остаётся stateless: состояние живёт в куке и на сервере, а не в протоколе.
- Практическое следствие — куки участвуют в кэшировании и CORS, поэтому их наличие меняет поведение промежуточных прокси и браузера.
