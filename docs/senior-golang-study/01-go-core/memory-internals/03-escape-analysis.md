...
- **`leaking param`** — *параметр* функции «утекает»: переданный в него указатель сохраняется куда-то, что живёт дольше вызова. Это влияет на вызывающую сторону: если передать `&x` в функцию с `leaking param`, то `x` тоже может escape.
