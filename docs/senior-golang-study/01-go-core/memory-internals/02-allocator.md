...
**Что это.** Когда в Go создаётся что-либо, выданная память **всегда заполнена нулями**. Никакого «мусора» из прошлого, как в C. Поэтому и существуют zero values:
...
В C `int x;` или `malloc()` отдают то, что лежало в этой памяти раньше (остатки чужих данных). В Go так не бывает — это и удобно (zero value сразу валиден), и безопасно (чужие данные не видны).
...
**1. Как устроен аллокатор памяти в Go?**
