...
Разница в **4000 раз**. 8 TB просто нельзя зарезервировать, а 2 GB виртуального адресного пространства — норма (и физической памяти займётся ещё меньше, т.к. многие стеки реально не вырастут).
...
|---|---|---|
| Когда OS-ядро освобождает страницы | немедленно | при нехватке памяти |
...
- после `MADV_FREE`: RSS не падает сразу (OS-ядро решает само), VSZ остаётся.
...
- ставь `GOMEMLIMIT` ниже `cgroup memory.limit` (обычно ~90% лимита) — тогда Go начнёт агрессивно собирать мусор и возвращать память **до** того, как OS-ядро убьёт процесс;
...
**3. Почему в проде OOMKilled, хотя памяти вроде хватает?**
