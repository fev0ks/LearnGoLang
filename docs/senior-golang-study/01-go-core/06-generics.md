...
Go выбрал середину. Он нанимает **одного работника на каждую «форму» типа** и выдаёт ему **шпаргалку (словарь, dictionary)** с конкретикой: «твой тип — 8 байт, копировать вот так, методы — вот в этой табличке». Работник общий, а детали подсматривает в шпаргалке.
