...
> Цифры абсолютны для конкретного CPU и будут другими у тебя — важны **соотношения**: для маленьких типов value не медленнее pointer и не аллоцирует, для больших — копия линейно дорожает, а взятие адреса добавляет аллокацию. Проверяй свой случай тем же `go test -bench -benchmem`.
