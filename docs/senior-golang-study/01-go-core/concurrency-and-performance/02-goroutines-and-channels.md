...
Если же нужен **простой 1:1 forwarder** без отмены и без conflation — гейт избыточен, хватает `for { v, ok := <-in; if !ok { return }; out <- v }`. Гейт оправдан ровно тогда, когда в `select` есть **что-то ещё** кроме одной отправки.
