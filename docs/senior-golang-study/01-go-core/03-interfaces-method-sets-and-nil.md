...
Видно три вещи: (1) `eface` (`any`) несёт **дескриптор типа** `type:int`, а `iface` — **`itab`** `go:itab.I,T`; (2) укладка **значения** (int, Celsius) вызывает боксинг (`convT64`), а укладка указателя — нет; (3) тип в type-слово грузится как **адрес символа**, заранее сгенерированного компилятором.
...
Частый вопрос на собеседовании: структуру кладут в один (узкий) интерфейс, потом приводят к другому интерфейсу — будут ли доступны методы, которых **не было** в первом интерфейсе, но которые есть у исходной структуры? 
