...
`errors.Is` даёт перехватить это сравнение: если у типа есть метод `Is(target error) bool`, движок вызовет **его** вместо `==`. Тип сам решает, что считать «совпадением».
...
**Зачем.** `errors.Is` отвечает на вопрос «**та ли это ошибка?**» — да/нет. Но если ошибка несёт данные (`Field`, `Status`, `Code`), часто нужно не просто узнать факт, а **достать сам объект** и прочитать его поля. Это и делает `errors.As`: разматывает цепочку, находит первый уровень нужного типа и **записывает его** в переданную переменную.
...
Если ошибка уже содержит нужный контекст и просто пробрасывается вверх:
