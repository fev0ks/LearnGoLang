...
`unsafe` — пакет, который выключает гарантии безопасности типов и памяти Go ради прямого доступа к layout'у. На нём построены `reflect`, `sync/atomic.Pointer`, zero-copy конверсии `string`↔`[]byte`, и куски рантайма. В прикладном коде нужен редко, но на senior-собесах про него спрашивают: чем `unsafe.Pointer` отличается от `uintptr`, почему GC может «увести» объект, и как устроены padding и alignment.
...
Ключевое следствие: как только `unsafe.Pointer` превращён в `uintptr`, **GC перестаёт считать это ссылкой**. Объект может быть собран сборщиком или сдвинут при росте стека — и `uintptr` будет указывать в мусор.
...
⚠️ **Цена:** нарушается immutability строки.
...
**Undefined behavior / порча памяти.** `float32` — 4 байта, `int64` — 8. Чтение `*(*int64)` по адресу `float32` залезает на 4 байта **за** переменную (в соседнюю память/padding). Под `checkptr` это поймается как ошибка.
