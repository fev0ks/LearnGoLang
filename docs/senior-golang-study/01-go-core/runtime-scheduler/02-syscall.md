...
M заходит в ядро и ждёт там; P через handoff уходит другому M. Тысячи таких параллельно → тысячи потоков (см. [Thread exhaustion](#thread-exhaustion-много-blocking-syscalls)).

```go
// 1. Чтение ОБЫЧНОГО файла. На Linux у regular-файлов нет O_NONBLOCK —
//    read() блокирует поток в ядре до конца чтения (диск/страничный кэш).
...
> Нюанс: в редких конфигурациях (источник тактов не поддерживает vDSO, некоторые гипервизоры) `clock_gettime` **откатывается на настоящий syscall** — тогда это обычный неблокирующий syscall с заходом в ядро. Но на обычном Linux/amd64/arm64 — vDSO.
...
### Кто будит M из syscall: ядро, а не Go

Важная деталь: всё выше (`exitsyscall`) — это то, что M делает **после** возврата из ядра. А **разбудить** его из блокирующего syscall Go **не может** — это делает ядро.

M — настоящий поток ОС, и в блокирующем `read()` он **спит внутри ядра** (в Linux — состояние `S`/`D`). Когда вызов может завершиться (пришли данные, истёк таймаут), **ядро** делает поток runnable, OS-планировщик ставит его на CPU, и поток возвращается в user-space → в `exitsyscall`. Go тут не участвует и **не контролирует, когда** это произойдёт.

Не путать два разных пробуждения M:

| M спит... | Кто будит | Механизм |
|---|---|---|
| **в syscall** (в ядре) | **ядро / OS-планировщик** | возврат из syscall |
| **припаркованным** в idle-пуле (`sched.midle`) | **рантайм Go** | `notewakeup` (futex) из `startm`, когда есть P + работа |

То есть из syscall M выходит сам, как только ядро вернёт управление; а уже припаркованный (после неудачного `exitsyscall`) M в следующий раз будит Go через futex.

### Почему `ctx` не отменяет застрявший syscall

Прямое следствие: **отмена `context` не освобождает M, висящий в блокирующем syscall.** Отмена `ctx` кооперативна — она лишь закрывает `ctx.Done()`, а горутина должна **сама** его проверять (`select`). Но горутина внутри syscall Go-код не исполняет — она в ядре, ни на каком `select` не стоит, и закрытие канала её «не слышит». M остаётся заблокированным, пока **ядро** не вернётся.

| I/O | `ctx`/deadline отменяет? | Почему |
|---|---|---|
| **сеть** (`conn.Read`, http) | **да** | горутина не в ядре, а **в netpoller**; рантайм будит её по дедлайну / при `conn.Close()` |
| **файл** (regular file) | **нет** | M реально висит в ядре на `read()` — прервать нельзя |
...
Отменить блокирующий вызов можно, только **заставив ядро вернуться**: закрыть fd (сокеты/pipe), убить процесс (exec), выставить deadline (сеть через netpoller). Для чтения обычного файла такого рычага нет.

> Опасное следствие: даже если на Go-уровне ты «отменишься» (обёртка-горутина вернётся по `ctx.Done()`), сам syscall и его M **остаются заблокированы** в ядре — ты просто перестал ждать результат. Это **утечка горутины + удержанный поток** до возврата ядра. Поэтому файловый/cgo I/O без ограничения параллелизма опасен, и `ctx` тут не помощник.
...
go func() {
    data, err := os.ReadFile(path) // ВСЁ РАВНО висит, пока ядро не вернётся
    ch <- result{data, err}
}()
...
case <-time.After(time.Second):
    ...
    // НО read, его горутина и поток (M) ещё заблокированы в ядре — это не отмена.
}
```
...
На входе выполняется `entersyscall`: сохраняются SP/PC горутины, статус → `Gsyscall`, P **отвязывается от M** (сразу через `entersyscallblock`, если вызов заведомо блокирующий, либо через ~20 мкс — sysmon retake). P подхватывает idle-M или создаётся новый — другие горутины продолжают работать. M висит в ядре с этой горутиной, без P. По завершении syscall выполняется `exitsyscall`, и **именно M** (вернувшийся из ядра поток — потоку нужен P, чтобы исполнять Go) пытается **взять P обратно**: если свободен — fast path, продолжает сразу; если нет — G уходит в global run queue, а **сам M паркуется**. Поэтому один blocking syscall не останавливает весь планировщик: P не ждёт M.

**2. «Какие операции блокируют поток (M), а какие нет?»**

**Блокируют M** (идут через blocking syscall path, держат поток в ядре): чтение **обычного файла** (`os.ReadFile`/`f.Read` — у regular-файлов нет O_NONBLOCK), запуск процесса (`exec` → `wait4`), **cgo**-вызовы. **Не блокируют M**: сетевой I/O (`conn.Read`/`Accept`) — сокет в O_NONBLOCK, горутина паркуется в netpoller, M свободен; таймеры/`time.Sleep` — per-P heap. **Вообще не syscall**: `time.Now()` — `clock_gettime` через vDSO, в ядро не заходит.

**3. «Чем vDSO отличается от обычного syscall?»**

Обычный syscall дорог из-за **mode switch** (переход user→kernel и обратно). **vDSO** — страница с кодом/данными, которую ядро отображает в адресное пространство процесса; «лёгкие» вызовы вроде `clock_gettime` выполняются **в user-space без захода в ядро** (читают обновляемую ядром страницу с часами). Поэтому `time.Now()` — это десятки нс, не syscall, и планировщика не касается.
