...
- **`runnext`** — один слот с наивысшим приоритетом. Туда попадает «горячая» горутина: и только что разблокированная (например, через channel send), и **только что созданная** через `go f()` (`newproc` зовёт `runqput(..., next=true)`). Если в `runnext` уже кто-то был — он **выталкивается в хвост LRQ**, а новый занимает слот. Идея — locality: только что созданную/разбуженную горутину выгоднее запустить сразу, она «горячая» в кэше;
...
### Пример: почему порядок «4 0 1 2 3» при GOMAXPROCS(1)

```go
func main() {
    runtime.GOMAXPROCS(1)        // один P → нет параллелизма, всё на одном потоке
    wg := sync.WaitGroup{}
    wg.Add(5)
    for i := 0; i < 5; i++ {
        go func(i int) {
...
Вывод **стабильно** (проверено на go1.26, GOMAXPROCS=1):
...
Почему так — ровно из-за `runnext` и «выталкивания» предыдущего жильца в хвост LRQ. Каждый `go func(i)` кладёт новую горутину в `runnext`, а ту, что там была, отправляет в конец LRQ:

```
go g0  → runnext=g0,  LRQ=[]
go g1  → runnext=g1,  LRQ=[g0]          (g0 вытолкнут в хвост)
go g2  → runnext=g2,  LRQ=[g0,g1]
go g3  → runnext=g3,  LRQ=[g0,g1,g2]
go g4  → runnext=g4,  LRQ=[g0,g1,g2,g3]
```

`main` сам ещё держит P и крутит цикл; отдаёт его только на `wg.Wait()` (блокируется). Тут планировщик выбирает следующую горутину по приоритету: **сперва `runnext` → это `g4`** (печатает `4`), затем LRQ с головы по FIFO → `g0, g1, g2, g3` (печатают `0 1 2 3`). Между короткими `Println` вытеснения нет — каждая горутина успевает отработать и завершиться. Отсюда `4 0 1 2 3`: «последняя созданная — первой исполнится».

⚠️ Это **деталь реализации, а не гарантия языка**: порядок выполнения горутин спецификацией не определён, полагаться на него нельзя. При `GOMAXPROCS > 1` появляются несколько P, work stealing и реальный параллелизм — порядок «поедет». Пример хорош именно как иллюстрация механики `runnext`, а не как контракт.
...
> **User-space vs kernel-space.** *User-space* — режим с ограниченными привилегиями, где исполняется обычный код (прикладная программа, рантайм Go): нет прямого доступа к железу и чужой памяти. *Kernel-space* — режим OS-ядра с полным доступом. Перейти из user в kernel можно только через **syscall** — это контролируемый mode switch, и он относительно дорогой. Поэтому «планировщик в user-space» = переключение горутин **не требует входа в OS-ядро** (в отличие от переключения потоков ОС, которым занимается OS-ядро). Подробно про режимы и mode switch — в [hardware-and-os/06-processes-and-threads](../../10-devops-and-observability/hardware-and-os/06-processes-and-threads.md#kernel-mode-vs-user-mode).
