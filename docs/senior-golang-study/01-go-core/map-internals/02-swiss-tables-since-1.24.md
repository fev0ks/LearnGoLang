...
Swiss Table — это hash table с open addressing: key/value лежат внутри массива slots, а collisions ищутся в следующих groups, а не в linked overflow buckets.

Главная оптимизация: рядом с каждой group из восьми slots хранится восемь маленьких control bytes. Runtime сначала сравнивает эти metadata, быстро получает несколько кандидатов и только затем читает и сравнивает полные keys.

Числовые constants и структуры ниже соответствуют реализации Go 1.26. Общая модель действует начиная с Go 1.24, но конкретный runtime layout не является API и может меняться.

## Содержание

- [Зачем hmap заменяют на Swiss Tables](#зачем-hmap-заменяют-на-swiss-tables)
...
## Зачем hmap заменяют на Swiss Tables

Старая `hmap` решает collisions через overflow chains. Это работает, но чтение длинной chain прыгает по pointers, хуже использует CPU cache и проверяет metadata slots по одному.

| Legacy hmap | Swiss Tables |
| --- | --- |
| Основной bucket и linked overflow buckets | Непрерывный массив groups внутри table |
| Pointer chasing при collisions | Open addressing и probing по индексам |
| `tophash` проверяется slot за slot | Восемь control bytes сравниваются как одно word |
| Grow постепенно эвакуирует общий bucket array | Небольшая table перестраивается целиком; большая map делится на tables |

Главный выигрыш даёт не «более умный hash», а организация памяти и уменьшение дорогих full-key comparisons. Особенно это заметно, когда key — string или большая struct: сначала runtime работает с компактными metadata и только потом обращается к key.

## Ментальная модель

Представьте парковку, разбитую на blocks по восемь мест:

- `hash(key)` подсказывает первый block;
- короткий fingerprint `H2` записан на табличке каждого занятого места;
- runtime одновременно сравнивает восемь табличек;
- только совпавшие fingerprints требуют сравнения полного key;
- если block не помог, probe sequence выбирает следующий.
...
Это всё ещё обычная hash table: средний lookup близок к `O(1)`, но collisions, заполненность и стоимость hash/equality влияют на реальное время.

## Group и control bytes

Текущая implementation группирует по восемь slots. Каждому slot соответствует control byte:
...
Для `map[uint8]uint64` interleaved layout может тратить padding на выравнивание `uint64`. Runtime осознанно принимает этот компромисс ради locality обычного lookup.
...
`0` появляется из-за wrap-around: для четырёх groups операция `& groupMask` эквивалентна modulo `4`, поэтому index после `3` возвращается к `0`.
...
Верхние directory bits входят в H1, но для group index используется другая его часть через маленький `groupMask`. Поэтому hash не обязательно делится на три строго непересекающихся поля.
...
- Не оптимизируйте application по `ctrl` bytes: сначала измерьте hash/equality, allocations и общий profile.
- Большой struct/string key дороже хэшировать и сравнивать; иногда полезен компактный stable identifier.
- Предварительный capacity hint уменьшает grows, если размер действительно известен.
- `delete` и `clear` не обещают уменьшить backing allocation; для release памяти заменяют map и измеряют эффект.
- Порядок `range` нельзя использовать как random shuffle — распределение не является API случайности.
- Runtime detection concurrent access не заменяет `Mutex` и race detector.
