...
Практические задачки на поведение map (одинаковое в обеих версиях) — в [03-puzzles-and-gotchas](./03-puzzles-and-gotchas.md).

**1. Что изменилось в map в Go 1.24?**
Реализацию заменили с `hmap + bmap` (chaining через overflow buckets) на **Swiss Tables** — open addressing с metadata-байтами. API и семантика не изменились, производительность +10–30%.

**2. Что такое ctrl-байты и matchH2?**
Каждый слот описывается одним `ctrl`-байтом: либо 7 бит `H2` (нижние биты хэша) у занятого, либо спец-значение (`empty 0x80` / `deleted 0xFE`). `matchH2` сравнивает H2 со всеми 8 ctrl-байтами **одной SWAR-операцией** (без цикла и branch), возвращая bitset совпадений.

**3. H1 vs H2?**
Хэш делится: `H1 = hash >> 7` выбирает группу (и table в directory), `H2 = hash & 0x7F` хранится в ctrl для быстрого отсева. Ложное совпадение 1/128.

**4. Зачем tombstones (ctrlDeleted)?**
При delete нельзя ставить `empty`, если probe-цепочка идёт дальше — иначе lookup не найдёт элементы за этой позицией. Ставим `deleted` (tombstone). Накопленные tombstones вычищаются при росте.

**5. Чем рост отличается от старого hmap?**
Раньше — инкрементальная эвакуация (1–2 bucket на операцию, lookup проверял оба массива). Теперь — **batch copy**: вся table перестраивается за раз, после чего чистая, без проверки oldbuckets при lookup. По latency не хуже: рост редок (load factor ~87.5%), а для больших map есть directory.

**6. Что за directory?**
Для больших map — массив указателей на независимые `table`. Растёт только перегруженная table, остальные не трогаются → ограничен пик памяти и время одного grow.

**7. Что НЕ изменилось?**
Всё пользовательское: случайный порядок итерации, паника при записи в nil map, `fatal` при concurrent-доступе, запрет `&m["key"]`, nil как zero value. Никаких breaking changes.
