...
**1. Что делает modular monolith настоящим, а не «папками»?**

- Архитектурные границы с enforcement: `module.go` — единственная точка входа модуля, всё остальное в `internal/` и недоступно снаружи; Go проверяет это на уровне компилятора.

**2. Как модули общаются между собой?**

- Три варианта: прямой вызов через интерфейс (синхронно, просто), domain events через in-process bus (decoupling, eventual consistency), shared read model через PostgreSQL view (read-heavy сценарии). Правило: модуль знает только интерфейс, не конкретную реализацию соседа.

**3. Как разделить базу данных?**

- PostgreSQL schemas по модулю — физическая изоляция без отдельных БД. Запреты: foreign keys между схемами (deployment coupling) и прямые JOIN в чужие схемы в production-коде (binding к чужой структуре).

**4. В чём главная ценность подхода?**

- Выделение модуля в сервис — изменение одного `main.go`: конкретный модуль заменяется HTTP-клиентом, реализующим тот же интерфейс.
