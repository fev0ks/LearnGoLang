...
REST — архитектурный стиль, а не протокол, и отсюда следует неприятное: неправильно спроектированный REST API работает. Можно отправлять POST вместо GET, класть глаголы в URL и путать query с body — сервер ответит, клиент разберёт ответ, тесты пройдут. Обратная связь появляется позже и в другом месте: у API, который нельзя кешировать; у клиента, который не может безопасно повторить запрос после таймаута; у команды, которая на каждый новый сценарий добавляет ещё один эндпоинт.

Статья разбирает правила проектирования и типичные ошибки, встречающиеся в работающих системах, — с объяснением, чем именно каждая из них оборачивается.

---

## Ресурсы, а не действия

Главная идея REST: URL адресует ресурс, а не команду.
...
| `PATCH` | Частично обновить ресурс | зависит от тела | ✗ | ✓ |
| `DELETE` | Удалить ресурс | ✓ | ✗ | — |

**Идемпотентный** — повторный вызов не меняет состояние сервера сильнее, чем первый. Речь о состоянии, а не об ответе: повторный `DELETE` вернёт другой код, но ресурс останется удалённым ровно один раз.

**Безопасный** — вообще не изменяет состояние сервера, поэтому запрос можно выполнить автоматически: по ссылке, из предзагрузки браузера, из поискового робота.

Идемпотентность PATCH зависит от содержимого тела, и в этом отличие от PUT. Тело `{"phone": "+7-999-..."}` задаёт конечное значение — повтор ничего не меняет. Тело вида `{"op": "increment", "field": "views"}` описывает операцию — каждый повтор увеличивает счётчик ещё раз. Практический вывод: если PATCH принимает инструкции, а не значения, повторять его после таймаута нельзя без ключа идемпотентности.

Свойства эти нужны не для чистоты. На них опираются сетевые посредники и клиентские библиотеки: безопасные и идемпотентные запросы автоматически повторяются при обрыве соединения, кешируются и предзагружаются. Метод, нарушивший обещание — `GET`, который списывает деньги, — ломается именно здесь: повтор случится без ведома автора кода.
...
| Body | Данные для создания или изменения ресурса | `POST /orders { items: [...] }` |

Разделение опирается на разницу в обязательности. Часть пути обязательна по определению: без идентификатора маршрут просто не совпадёт, и обработчик не получит управление. Query-параметр опционален всегда — его отсутствие для маршрутизатора нормально, и проверять его приходится в коде. Поэтому идентификатор в query порождает лишнюю ветку обработки и лишний способ ошибиться, а идентификатор в пути — нет.
...
Вариант 1 — переосмыслить параметры. Часто «сложный объект» — это несколько query-параметров:
...
Вариант 2 — если параметры действительно сложные, ввести ресурс-поиск:
...
Один стандарт для всего API. Большинство руководств по стилю рекомендуют plural везде:
...
Только kebab-case в сегментах пути:
...
Проблема: для работы с одним заказом клиент обращается к четырём разным «сервисам» с разными базовыми путями. Фронтенд вынужден знать внутреннюю структуру бэкенда. При ошибке непонятно, какой сервис виноват.
...
Паттерн — sub-resource action: вложенный ресурс-существительное, описывающий результат действия.
...
gRPC-gateway транслирует HTTP в gRPC, и в аннотации `google.api.http` поле `body` для метода `get` не задаётся: параметры запроса переносятся в путь и query string.

Ограничение это не изобретение gateway, а отражение спецификации. RFC 9110 не запрещает тело у GET напрямую, но говорит, что у него нет общепринятой семантики, оно не может изменить смысл запроса, а часть реализаций такой запрос отклонит. На практике это означает, что тело GET-запроса теряется непредсказуемо: одни прокси его пропускают, другие обрезают, третьи разрывают соединение.

Отсюда возникает искушение использовать POST для запросов, которые по смыслу являются чтением, но требуют сложных параметров.
...
Некоторые зрелые API (GitHub, Stripe) возвращают голые объекты для простых ресурсов и добавляют envelope только для коллекций. Это тоже валидный подход — главное — придерживаться его во всём API.
...
Три названия для одного и того же — списка элементов. Клиент вынужден каждый раз сверяться с документацией, а генератор SDK выдаёт три несвязанных типа вместо одного.
...
Если эндпоинт требует авторизации, идентификатор пользователя уже есть в токене. Передавать его ещё и в теле мешает по трём причинам:

- **Неоднозначность.** Неясно, кто отправитель — тот, что в токене, или тот, что в теле, и что делать, если они разные.
- **Уязвимость.** Клиент подставляет чужой идентификатор и получает чужие данные — ровно в тот момент, когда кто-то забыл сверить его с токеном. Проверка «поле должно совпадать с токеном» существует только потому, что поле вообще есть.
- **Два источника правды.** Идентификатор берётся из двух мест, и рано или поздно они разойдутся.

**Правильно:**

Для авторизованных эндпоинтов идентификатор пользователя берётся на сервере из токена, а в контракте API его нет вовсе. Поля, которого нет, нельзя подделать — это сильнее любой проверки.
...
|---|---|---|
| `payedAt` | `paid_at` | `payed` — неверная форма, правильно `paid` |
...
- Главное — строить вокруг ресурсов-существительных: `/orders/123` адресует заказ, действие выражает HTTP-метод.
- Именование — множественное число (`/orders`, не `/order/list`), kebab-case (`/contact-info`), идентификатор в пути, а не в query.
- Переходы состояния — вложенным ресурсом: `POST /payments/{id}/confirmation` создаёт подтверждение, а не вызывает метод.
- Критерий качества — запрос читается без документации: `GET /users/123/orders?status=pending` понятен сам по себе.

**2. Зачем нужны свойства «безопасный» и «идемпотентный»?**

- Причина — на них опираются посредники: такие запросы кешируются, предзагружаются и автоматически повторяются при обрыве связи.
- Идемпотентность — про состояние сервера, а не про ответ: повторный `DELETE` вернёт 404, но удаление произойдёт один раз.
- PATCH — идемпотентен, только если тело задаёт значения; тело с инструкцией вида «увеличить счётчик» повторять нельзя.
- Цена нарушения — `GET`, который меняет состояние, сработает повторно сам, без участия автора кода.

**3. Какие частые ошибки в дизайне URL?**

- Глаголы в пути — `/orders/confirm` вместо `POST /orders/123/confirmation`.
- POST для чтения — теряются кеширование и возможность безопасного повтора.
- Суффикс `/list` на коллекции — `/orders` уже коллекция, а два URL для одного ресурса удивляют клиента.
- Идентификатор в query — параметр формально опционален, поэтому появляется лишняя ветка обработки.
- Аббревиатуры (`/rec`, `/acc`) — экономят символы и стоят вопроса в каждом обсуждении.

**4. Какие типичные проблемы в дизайне ответа?**

- Даты как unix timestamp — непонятно, секунды это или миллисекунды и в какой зоне; ISO 8601 отвечает на оба вопроса сам.
- Деньги как число с плавающей точкой — потеря точности; нужны минорные единицы и явная валюта.
- Внутренние статусы наружу — `BookingFinishedWithError` не говорит клиенту, что показать пользователю.
- Непоследовательная структура — где-то `list`, где-то `orders`, где-то голый массив: генератор SDK выдаёт три разных типа.
- Отсутствие пагинации на растущей коллекции и поля `object`/`any` без схемы.
- Идентификатор пользователя в теле авторизованного запроса — источник правды один, и это токен.
