...
- От проблемы: простой сервис — layered достаточно; сложная domain-логика или несколько входных каналов — hexagonal; надёжно связать DB update с publish события — outbox; распределённый процесс — saga с явным state machine и idempotency.

**2. Какие паттерны чаще всего недооценивают?**

- Level-triggered reconciliation — когда нужна устойчивость к потере событий и partial failures (система сверяет желаемое состояние с фактическим, а не реагирует на каждое событие), и strangler fig — когда legacy нельзя переписать сразу и новую функциональность выносят постепенно.

**3. Какой главный критерий, что паттерн выбран правильно?**

- Он снижает стоимость изменений и эксплуатации, а не добавляет церемонию ради правильного названия.
