...
**1. Как SOLID выражается в Go без классов и наследования?**

- SRP — разделение на слои: handler меняется от API, service — от бизнес-правил, repository — от схемы хранения (свалка `utils` — классическое нарушение). OCP — расширение через интерфейсы: новый тип Sender добавляется без правки существующего кода; middleware-цепочка — тот же принцип. LSP — реализация интерфейса ведёт себя предсказуемо: не паникует, корректно обрабатывает ошибки, уважает context (`io.Reader` — образец: файл, буфер, gzip взаимозаменяемы).

**2. Почему ISP в Go особенно естественен?**

- Интерфейс объявляет потребитель с минимумом методов — duck typing делает это бесплатным. `io.Reader`/`io.Writer` — по одному методу; жирный `UserRepository` на 8 методов заставляет каждый mock реализовывать всё — лучше три узких интерфейса.

**3. Как выглядит DIP в Go?**

- Интерфейс объявляется в пакете потребителя (`domain.OrderRepository`), реализация — в инфраструктурном (`postgres.pgOrderRepo`). Зависимость инвертирована: postgres зависит от domain, не наоборот — это и есть основа тестируемости без реальной БД.
