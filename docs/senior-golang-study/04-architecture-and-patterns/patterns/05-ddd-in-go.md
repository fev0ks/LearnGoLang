...
Domain-Driven Design — подход к проектированию, при котором центром архитектуры становится модель предметной области, а не схема базы или структура HTTP-эндпоинтов. Практическая цель у него одна: сделать так, чтобы бизнес-правило жило в одном месте и его нельзя было обойти.

В Go это ложится на язык естественнее, чем в языках с наследованием. Закрытые (unexported) поля структуры не позволяют изменить состояние мимо метода; интерфейс, объявленный в пакете-потребителе, разворачивает зависимость от инфраструктуры к домену; отдельный пакет с `internal/` даёт физическую границу контекста. Иерархии классов, фабрики фабрик и слой спецификаций при этом не нужны.

Материал разбит на два уровня: стратегический — как разрезать систему на части, тактический — из каких блоков собирается модель внутри одной части.
...
Каждый bounded context — это отдельная модель и отдельный код; единый объект `User` на все случаи жизни создавать не нужно.

Причина не в чистоте, а в скорости изменений. Общий `User` собирает в себе поля всех контекстов сразу: пароль и роли из Accounts, адрес доставки из Orders, платёжные реквизиты из Billing. Дальше любое изменение в биллинге затрагивает структуру, которую читает авторизация, а тест на выставление счёта требует заполнить поля, к счетам отношения не имеющие. Разделение контекстов делает эти изменения независимыми ценой дублирования идентификатора и части полей.
...
| Паттерн интеграции | Смысл | Цена |
|---|---|---|
| Shared Kernel | Общий код (типы, логика) для тесно связанных контекстов одной команды | Любое изменение согласуется обеими сторонами |
| Customer-Supplier | Поставщик публикует API и учитывает потребности потребителя | Поставщик обязан считаться с чужим планом |
| ACL (Anti-Corruption Layer) | Слой-переводчик защищает свою модель от чужой | Дополнительный код преобразования |
| Open Host Service | Публичный API для множества потребителей, не подстраивается под каждого | Потребитель не влияет на контракт |
| Conformist | Модель поставщика принимается как есть | Чужие решения протекают в свой домен |

Выбор между ними определяется тем, кто кому может диктовать условия. Если поставщик — внешний сервис, повлиять на его контракт нельзя, и остаётся выбор между ACL (платим кодом преобразования, сохраняем свою модель) и Conformist (не платим ничем сейчас, платим потом при каждом изменении на их стороне).
...
**Aggregate** — кластер Entity и Value Object, объединённых единой границей транзакционной согласованности. Один из объектов, **Aggregate Root**, служит единственной точкой входа.

**Главное правило:** одна транзакция изменяет один aggregate.

Правило выглядит произвольным, пока не посмотреть, что оно покупает. Граница агрегата — это ответ на вопрос «что обязано быть согласованным в один и тот же момент времени». Инвариант «сумма заказа равна сумме позиций» обязан выполняться всегда, поэтому позиции и сумма лежат внутри одного агрегата и меняются одной транзакцией. Инвариант «у заказа есть оплата» такой срочности не требует: между размещением заказа и списанием денег проходит время, и система это переживает.

Отсюда следствия, которые видны в эксплуатации:

- Транзакция, меняющая один агрегат, блокирует ровно его строки. Транзакция, меняющая три, удерживает блокировки дольше и сталкивается с другими такими же — растёт число взаимных блокировок и время ожидания.
- Крупный агрегат («заказ вместе с доставками, платежами и возвратами») превращает любое изменение любой его части в конфликт со всеми остальными изменениями. Смена статуса доставки ждёт, пока завершится расчёт возврата.
- Согласованность между агрегатами достигается иначе — событиями и повторами, а не общей транзакцией. Это осознанный обмен строгой согласованности на возможность менять части независимо.
...
func (o *Order) AddItem(productID ProductID, qty int, price Money) error {
    ...
    // Инвариант заказа: все позиции в одной валюте.
    // Проверка здесь, а не в вызывающем коде, — иначе её можно обойти.
    if len(o.items) > 0 && o.items[0].price.Currency() != price.Currency() {
        return fmt.Errorf("%w: order is in %s, item is in %s",
            ErrCurrencyMismatch, o.items[0].price.Currency(), price.Currency())
    }
...
// recalcTotal — валюта берётся из позиций заказа, а не задаётся константой:
// иначе заказ в EUR молча посчитается как USD.
func (o *Order) recalcTotal() error {
    ...
    for _, item := range o.items {
        sum, err := total.Add(item.price.Multiply(int64(item.quantity)))
        if err != nil {
            return fmt.Errorf("recalc total: %w", err)
        }
        total = sum
...
Копия в `Items()` защищает не от чтения, а от изменения в обход методов. `append([]OrderItem{}, o.items...)` создаёт новый массив, поэтому запись `order.Items()[0].quantity = 100` меняет копию и до состояния заказа не доходит — сумма остаётся согласованной. Копия здесь поверхностная, и этого достаточно, пока поля `OrderItem` — значения. Появится в нём слайс или указатель — вместе с ним появится и путь к внутреннему состоянию, и копирование придётся делать глубже.

### Одновременное изменение одного агрегата

Схема «загрузили → изменили в памяти → сохранили целиком» вводит окно, в котором два обработчика читают одно состояние и записывают разные результаты. Второй `Save` затирает первый, и потеря выглядит безобидно: ошибки нет, лог чист, в базе просто нет одной из двух правок.

Защита — версия агрегата и условие в `UPDATE`:

```go
// Version — часть состояния агрегата, увеличивается при каждом сохранении
func (o *Order) Version() int { return o.version }

// В репозитории:
tag, err := r.db.Exec(ctx, `
    UPDATE orders
       SET status = $1, total_amount = $2, version = version + 1, updated_at = NOW()
     WHERE id = $3 AND version = $4
`, order.Status(), order.Total().Amount(), order.ID(), order.Version())
if err != nil {
    return fmt.Errorf("save order: %w", err)
}
if tag.RowsAffected() == 0 {
    // Версия в базе уже другая: между чтением и записью кто-то успел сохранить.
    return domain.ErrConcurrentModification
}
```

Вызывающий код на `ErrConcurrentModification` перечитывает агрегат и повторяет операцию. Такой подход называют оптимистической блокировкой: конфликт не предотвращается, а обнаруживается. Он выигрывает у `SELECT FOR UPDATE`, пока конфликты редки — не удерживаются блокировки на время бизнес-логики. При частых конфликтах повторы начинают съедать больше, чем экономят, и пессимистическая блокировка становится дешевле.
...
Разница между подходами — в том, что происходит при падении процесса между сохранением и публикацией.

| | Outbox (подход 1) | EventBus в памяти (подход 2) |
|---|---|---|
| Событие и состояние | В одной транзакции | Разными операциями |
| Падение между ними | Невозможно: либо оба, либо ничего | Состояние сохранено, событие потеряно навсегда |
| Гарантия доставки | At-least-once | At-most-once |
| Что нужно рядом | Таблица outbox и процесс-публикатор | Ничего |
| Где уместно | Межсервисное взаимодействие | Модульный монолит, где потеря события переживаема |

Второй подход не «неправильный» — он честно дешевле и подходит для событий, потеря которых не ломает состояние: обновление кеша, отправка метрики, необязательное уведомление. Как только по событию списываются деньги или меняется состояние в другом сервисе, потеря перестаёт быть допустимой, и нужен outbox — механика разобрана в [Saga и Outbox](./09-saga-and-outbox.md).
...
**Важно:** Repository работает с aggregate целиком — метода `UpdateStatus(id, status)` в нём нет. Порядок всегда один: загрузить → изменить методом агрегата → сохранить.

Причина в том, что точечное обновление обходит инварианты. `UpdateStatus(id, "cancelled")` выставит статус, не спросив, отгружен ли уже заказ, — вся защита, написанная в `Cancel`, окажется бесполезной, потому что рядом появился второй путь к тому же полю. Правило «репозиторий работает агрегатом целиком» существует для того, чтобы такого второго пути не было.

Отдельно стоит различать `reconstitute` и конструктор `NewOrder`. Конструктор создаёт новый заказ и проверяет входные данные: пустой идентификатор — ошибка. `reconstitute` восстанавливает уже существовавший заказ из строк таблицы и валидацию не повторяет — данные прошли её при создании, а повторная проверка сделала бы невозможной загрузку записей, сохранённых по прежним правилам.
...
**Практическое правило:** если всю логику можно описать словами «проверить и сохранить», нужен CRUD, а не DDD. Если есть состояния, переходы между ними и инварианты вроде «заказ нельзя отменить после отгрузки» — DDD оправдан.

Стоит держать в голове и обратную сторону. DDD-модель платит за защиту инвариантов многословностью: закрытые поля требуют конструкторов и геттеров, восстановление из базы — отдельного маппинга, каждое изменение состояния — метода. На CRUD-сервисе эта цена не окупается ничем: там нет инварианта, который стоило бы защищать, и слой преобразований просто удлиняет путь от запроса до строки в таблице.
...
**1. Что такое стратегический DDD?**

- Главное — разрезать систему так, чтобы внутри каждой части термин имел один смысл: это Bounded Context.
- Ubiquitous Language — тот же словарь в коде и в разговоре с бизнесом: `Cancel(reason)`, а не `UpdateStatus(3)`.
- Между контекстами — явный контракт: ACL, Open Host Service, Customer-Supplier.
- В Go — пакет с `internal/`-границей: связь через публичный API соседа, а не через его внутренние типы.

**2. Какие есть тактические строительные блоки?**

- Entity — тождество по идентификатору, состояние меняется, объект остаётся тем же.
- Value Object — тождество по значению, неизменяемый: Money, Email, Address.
- Aggregate — граница транзакционной согласованности с единственной точкой входа.
- Domain Event — свершившийся факт в прошедшем времени: OrderPlaced.
- Repository — интерфейс в домене, реализация в инфраструктуре, работа агрегатом целиком.
- Application Service — оркестрация сценария без единого бизнес-правила внутри.

**3. Почему одна транзакция меняет один агрегат?**

- Причина — граница агрегата и есть ответ на вопрос, что обязано быть согласованным одномоментно.
- Следствие для базы — короткие блокировки на предсказуемом наборе строк вместо взаимных блокировок.
- Следствие для крупного агрегата — изменения его частей конфликтуют между собой, хотя друг с другом не связаны.
- Между агрегатами — согласованность через события и повторы, то есть отложенная.

**4. Как DDD ложится на Go?**

- Закрытые поля — инвариант нельзя обойти: состояние меняется только методом агрегата.
- Интерфейс в пакете домена — зависимость развёрнута, инфраструктура зависит от домена, а не наоборот.
- События — накапливаются в поле агрегата и публикуются Application Service после сохранения.
- Одновременные изменения — версия агрегата и `UPDATE ... WHERE version = $n`, иначе второе сохранение молча затирает первое.

**5. Когда DDD не нужен?**

- Признак — вся логика описывается словами «проверить и сохранить».
- Цена подхода — конструкторы, геттеры, отдельный маппинг при восстановлении из базы; на CRUD она не окупается.
- Когда окупается — есть состояния с переходами, инварианты и правила, которые регулярно меняются.
- Дополнительный аргумент — несколько команд с разными моделями одного слова и высокая цена ошибки в логике.
