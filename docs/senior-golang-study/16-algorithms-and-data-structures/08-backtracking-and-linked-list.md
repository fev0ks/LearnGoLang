# Перебор с возвратом и связные списки

## Содержание

- [Перебор с возвратом](#перебор-с-возвратом)
  - [Идея](#идея)
  - [Универсальный шаблон](#универсальный-шаблон)
  - [Задача 1: все перестановки](#задача-1-все-перестановки)
  - [Задача 2: все подмножества](#задача-2-все-подмножества)
  - [Задача 3: сумма с повторным использованием](#задача-3-сумма-с-повторным-использованием)
  - [Задача 4: поиск слова в сетке](#задача-4-поиск-слова-в-сетке)
  - [Задача 5: расстановка ферзей](#задача-5-расстановка-ферзей)
  - [Отсечение: ключевые приёмы](#отсечение-ключевые-приёмы)
- [Связные списки](#связные-списки)
  - [Определение узла](#определение-узла)
  - [Техники](#техники)
  - [Задача 1: развернуть список](#задача-1-развернуть-список)
  - [Задача 2: найти середину](#задача-2-найти-середину)
  - [Задача 3: начало цикла](#задача-3-начало-цикла)
  - [Задача 4: слияние отсортированных списков](#задача-4-слияние-отсортированных-списков)
  - [Задача 5: удалить N-й с конца](#задача-5-удалить-n-й-с-конца)
  - [Задача 6: проверка на палиндром](#задача-6-проверка-на-палиндром)
- [Сводная таблица сложности](#сводная-таблица-сложности)
- [Частые ошибки на интервью](#частые-ошибки-на-интервью)
- [Interview-ready answer](#interview-ready-answer)

Два классических раздела алгоритмических интервью. Перебор с возвратом (backtracking) — систематический обход всех вариантов с откатом решений, применяется к перестановкам, подмножествам и поиску путей. Связные списки — набор приёмов работы с указателями: фиктивная голова, два указателя разной скорости, обнаружение цикла.
...
- Структура одна и та же — обход дерева решений, — но перебор с возвратом отсекает ветви, которые заведомо не приведут к ответу.
- Отсечение не меняет асимптотику: она остаётся O(2^n) для подмножеств и O(n!) для перестановок.
- На практике разница огромна: в задаче о расстановке ферзей проверка диагоналей отбрасывает подавляющее большинство ветвей ещё до глубокого спуска.
- Обязательный элемент шаблона — откат: после рекурсивного вызова состояние возвращается в исходное, иначе ветви начнут влиять друг на друга.

**2. Какая ошибка в Go-реализации перебора встречается чаще всего?**

- Сохранение текущего пути в результат без копирования: срез разделяет базовый массив, и все накопленные ответы окажутся одинаковыми.
- Правильно — `append([]int{}, path...)` или `slices.Clone(path)` при добавлении в результат.
- Вторая по частоте — забытый откат изменённых данных, например клетки поля в задаче поиска слова.
- Третья — передача пути по значению без понимания, что `append` может выделить новый массив, из-за чего откат перестаёт работать.

**3. Зачем нужна фиктивная голова в задачах на связные списки?**

- Она устраняет особый случай «удаляем или вставляем перед первым элементом», который иначе требует отдельной ветки.
- Код становится однородным: указатель на предыдущий узел существует всегда, даже для первого.
- Возвращается при этом `dummy.Next`, а не `dummy`, — это отдельный источник ошибок.
- Цена — один лишний узел на стеке, что для задачи собеседования несущественно.

**4. Как найти начало цикла, а не только факт его наличия?**

- Первый этап — обычный алгоритм Флойда: медленный и быстрый указатели встречаются внутри цикла.
- Второй этап — один указатель возвращается в голову списка, и дальше оба двигаются по одному шагу за итерацию.
- Точка их повторной встречи и есть начало цикла: расстояние от головы до входа равно расстоянию от точки встречи до входа по циклу.
- Всё выполняется за O(n) по времени и O(1) по памяти, тогда как решение через множество посещённых узлов стоило бы O(n) памяти.

**5. Как удалить N-й узел с конца за один проход?**

- Быстрый указатель уходит вперёд на N+1 шаг, после чего оба двигаются синхронно до конца списка.
- Когда быстрый достигает конца, медленный стоит непосредственно перед удаляемым узлом.
- Именно N+1, а не N: при сдвиге ровно на N медленный останавливается на самом удаляемом узле, и предыдущий будет недоступен.
- Фиктивная голова здесь обязательна: без неё удаление первого узла требует отдельной ветки.
