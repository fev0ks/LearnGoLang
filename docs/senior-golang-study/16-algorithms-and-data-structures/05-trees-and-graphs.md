...
- В ширину — когда важна длина пути: он обходит вершины строго по возрастанию расстояния, поэтому первый достигнутый путь и есть кратчайший в невзвешенном графе.
- В глубину — когда важен факт достижимости, связность или структура: компоненты, обнаружение цикла, топологический порядок.
- По памяти они расходятся: в ширину хранится целый уровень, то есть до O(ширины), в глубину — стек глубиной O(высоты).
- На вырожденном дереве-цепочке рекурсивный обход в глубину даёт O(n) кадров стека, и это реальная причина переполнения на больших входных данных.
...
- Свойство дерева поиска глобальное: любой узел левого поддерева должен быть меньше всех предков выше, а не только своего родителя.
- Контрпример на трёх уровнях: корень 10, слева 5, у пятёрки справа 12 — все локальные сравнения проходят, но 12 нарушает границу корня.
- Правильный подход — передавать вниз по рекурсии допустимый диапазон и сужать его на каждом шаге.

**3. Как обнаружить цикл и чем случаи ориентированного и неориентированного графа отличаются?**

- В ориентированном — обход в глубину с тремя состояниями вершины: не посещена, в текущем стеке рекурсии, полностью обработана. Ребро в вершину из текущего стека и есть цикл.
- Отметки «посещена» недостаточно: повторный вход в уже обработанную вершину означает лишь повторный путь, а не цикл.
- В неориентированном — проще через систему непересекающихся множеств: если концы ребра уже в одном множестве, ребро замыкает цикл.
- Алгоритм Кана даёт то же самое побочным эффектом: если обработано меньше вершин, чем есть в графе, оставшиеся лежат в цикле.

**4. Что делает алгоритм Кана и почему он же отвечает на вопрос о цикле?**

- Считает число входящих рёбер у каждой вершины и начинает с тех, у кого их нет — это задачи без зависимостей.
- При обработке вершины у соседей уменьшается счётчик, и вершина с обнулившимся счётчиком попадает в очередь.
- Если в конце обработаны все вершины, топологический порядок существует; если меньше — часть графа взаимно заблокирована, то есть содержит цикл.
- Сложность O(V + E): каждая вершина и каждое ребро рассматриваются ровно один раз.

**5. Почему операции системы непересекающихся множеств считаются почти константными?**

- Две оптимизации работают вместе: сжатие пути при поиске корня и присоединение меньшего дерева к большему по рангу.
- Сжатие пути делает последующие обращения к тем же элементам прямыми, поэтому амортизированная стоимость размазывается по серии операций.
- Формально это O(α(n)), где α — обратная функция Аккермана: для любых реальных `n` она не превышает четырёх.
- Без обеих оптимизаций дерево может выродиться в цепочку, и операция станет O(n).
