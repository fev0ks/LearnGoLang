...
**1. В каких реальных сценариях MongoDB оправдана?**

- Когда домен действительно документный: product catalog с per-category атрибутами, user profile с flexible extensions от разных команд, CMS с составными blocks, activity feed с гетерогенными event payload. Общий признак — объект читается и пишется целиком, а атрибуты сущностей сильно различаются.

**2. Какой главный антипаттерн выбора MongoDB?**

- «Не хочется миграций»: проблема схемы не исчезает, а перекладывается с DDL на application-level validation — каждая версия структуры теперь живёт в коде всех потребителей.

**3. Когда вместо MongoDB достаточно PostgreSQL + JSONB?**

- Когда flexible нужна только часть данных, а основная модель реляционная: JSONB-колонка рядом с обычными таблицами даёт гибкость без второй СУБД в стеке и проще в эксплуатации.
