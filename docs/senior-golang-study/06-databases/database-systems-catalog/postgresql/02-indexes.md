...
Для геометрии (PostGIS), диапазонов (`tsrange`, `int4range`), similarity (`pg_trgm`), KNN и exclusion-constraint. Это не конкретный индекс, а **каркас сбалансированного дерева**: ты задаёшь, что значит «ключ» и «пересечение» через support-функции — и получаешь индекс под свой тип данных.
