...
**Failsafe-режим.** Если таблица всё же подобралась к wraparound (см. ниже), autovacuum переключается в аварийный режим: при достижении `vacuum_failsafe_age` (по умолчанию 1.6 млрд) он **пропускает зачистку индексов и throttling** (`cost_delay`) и гонит только заморозку, лишь бы успеть до катастрофы. Это страховка, а не штатный режим — если она срабатывает, autovacuum настроен слишком вяло.
