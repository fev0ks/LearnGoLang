...
**UUID v7** — рекомендуемый выбор: временная компонента + random, монотонно возрастают в пределах миллисекунды, поэтому ложатся в B-tree почти как последовательные (в отличие от v4, который рандомен и рвёт локальность вставки):
```sql
-- UUID v4 (случайный) — в ядре с PostgreSQL 13, расширения не нужны
SELECT gen_random_uuid();

-- UUID v7 (time-ordered) — встроенная функция появилась в PostgreSQL 18
SELECT uuidv7();

-- до PG18 v7 генерируют на стороне приложения (например, github.com/google/uuid: uuid.NewV7())
-- расширение uuid-ossp умеет только v1/v3/v4/v5 — v7 в нём НЕТ
```
...
**Шардировать по неравномерному ключу** — шардирование по `country` даст огромный шард для US и маленький для LU. Используй ключи с равномерным распределением.
...
**1. Чем шардирование отличается от партиционирования?**

- Партиционирование — один сервер (решает размер индексов и скорость vacuum); шардирование — несколько независимых серверов (решает write throughput и total storage), но теряет кросс-шардовые ACID-транзакции и JOIN.

**2. Когда вообще шардировать?**

- В последнюю очередь — после индексов, партиционирования, read replicas, pooling и вертикального масштабирования; когда write throughput или объём не тянет один сервер.

**3. Какие способы шардирования?**

- Application-level (hash — простой, но resharding двигает почти все ключи; range — удобно архивировать, но ловит hotspot; consistent hashing — двигается ~1/N ключей) и Citus (прозрачное, `colocate_with`, reference-таблицы).

**4. Главная боль шардирования?**

- Кросс-шардовые транзакции: нет ACID без 2PC → Saga/Outbox; схему проектируют так, чтобы связанные данные были на одном шарде (по tenant_id/user_id).

**5. Что с глобальными ID?**

- `SERIAL` коллизит между шардами → UUIDv7 или Snowflake, переходить ДО шардирования.

**6. Частые антипаттерны?**

- Шардировать преждевременно, по неравномерному ключу (`country`), с кросс-шардовыми транзакциями везде, без мониторинга hotspot.
