...
Уровень изоляции отвечает на два вопроса:

1. Когда PostgreSQL фиксирует снимок (`snapshot`) видимых данных.
2. Может ли набор конкурентных транзакций успешно закоммититься, если их
   результат нельзя получить последовательным выполнением.

Чем выше уровень, тем больше гарантий, но это не означает, что PostgreSQL
блокирует все конкурентные операции. На строгих уровнях он может обнаружить
конфликт, прервать транзакцию с `40001 serialization_failure` и потребовать от
приложения повторить её целиком.

| Уровень | Какой снимок видит транзакция | От чего защищает | Что всё ещё возможно |
| --- | --- | --- | --- |
| `READ UNCOMMITTED` | В PostgreSQL работает как `READ COMMITTED`. | Даёт те же гарантии, что `READ COMMITTED`; отдельной более слабой семантики нет. | То же, что на `READ COMMITTED`. |
| `READ COMMITTED` (по умолчанию) | Новый снимок на каждый SQL-оператор. | От dirty read: незакоммиченные изменения другой транзакции не видны. | Между двумя операторами возможны non-repeatable read и phantom read; многошаговая проверка бизнес-инварианта может закончиться serialization anomaly, например write skew. |
| `REPEATABLE READ` | Один снимок с начала первого оператора с данными и до конца транзакции. | От dirty read, non-repeatable read и phantom read. Повторные запросы читают согласованный снимок. | Возможна serialization anomaly, в том числе write skew: стабильный снимок ещё не делает конкурентный результат эквивалентным последовательному выполнению. |
| `SERIALIZABLE` | Такой же стабильный снимок, как на `REPEATABLE READ`, плюс проверка зависимостей между транзакциями. | От всех перечисленных аномалий, включая write skew: результат успешно завершившихся транзакций эквивалентен некоторому последовательному порядку. | Вместо аномального `COMMIT` одна из конфликтующих транзакций может получить `40001`; приложение обязано повторить её целиком. |

### `READ COMMITTED`: согласованность одного оператора

Один `SELECT` не видит незакоммиченных данных и работает с согласованным снимком.
Но следующий `SELECT` в той же транзакции получает уже новый снимок и может
увидеть чужой `COMMIT`. Поэтому стабильность видимых данных ограничена одним
`SELECT` и не распространяется на цепочку «прочитать → принять решение в Go →
записать другим оператором».

### `REPEATABLE READ`: стабильный снимок транзакции

Все запросы читают один снимок, поэтому повторное чтение строки или диапазона не
меняется из-за конкурентных `COMMIT`. В PostgreSQL это защищает и от phantom
read, хотя стандарт SQL допускает более слабую реализацию этого уровня.

Граница гарантии проходит по бизнес-инвариантам между несколькими строками. Две
транзакции могут принять решения по одному стабильному состоянию, изменить
разные строки и вместе нарушить правило — это write skew. Кроме того, если
транзакция попытается изменить строку, которую конкурент уже изменил после
создания её снимка, PostgreSQL прервёт транзакцию с `40001`; нужно повторить всю
операцию с новым снимком.

### `SERIALIZABLE`: корректность всего конкурентного результата

PostgreSQL разрешает закоммититься только такому набору `SERIALIZABLE`-транзакций,
для которого существует эквивалентный последовательный порядок. Поэтому уровень
защищает не только повторные чтения, но и бизнес-правила вида «посчитать строки,
проверить лимит, затем изменить другую строку».

Защита достигается не ожиданием всех конкурентов, а обнаружением опасных
зависимостей: одна из транзакций может получить `40001`. Гарантия имеет смысл,
когда все операции, поддерживающие один инвариант, используют `SERIALIZABLE` или
совместимые явные блокировки, а код умеет безопасно повторять транзакцию целиком.

`READ UNCOMMITTED` отдельно разбирать не нужно: PostgreSQL принимает такое имя,
но фактически выполняет транзакцию как `READ COMMITTED`. Поэтому dirty read в
PostgreSQL невозможен ни на одном уровне.
...
**Защита по уровням:** все уровни PostgreSQL, включая запрошенный
`READ UNCOMMITTED`, защищают от dirty read.
...
**Защита по уровням:** `READ COMMITTED` допускает non-repeatable read;
`REPEATABLE READ` и `SERIALIZABLE` предотвращают его.
...
**Защита по уровням:** `READ COMMITTED` допускает phantom read;
`REPEATABLE READ` и `SERIALIZABLE` предотвращают его в PostgreSQL.
...
**Защита по уровням:** `READ COMMITTED` и `REPEATABLE READ` допускают write
skew; `SERIALIZABLE` предотвращает его, откатывая одну из конфликтующих
транзакций.
...
**Защита по уровням:** `READ COMMITTED` и `REPEATABLE READ` допускают
serialization anomaly; `SERIALIZABLE` предотвращает её для успешно
закоммиченных транзакций.
...
- `READ COMMITTED` — новый снимок на каждый оператор; защищает от dirty read,
  но допускает non-repeatable read, phantom read и write skew.
- `REPEATABLE READ` — один снимок на транзакцию; защищает от dirty,
  non-repeatable и phantom read, но допускает write skew.
- `SERIALIZABLE` — добавляет SSI и защищает от write skew и других serialization
  anomalies ценой возможного отката с `40001` и повтора всей транзакции.
- `READ UNCOMMITTED` — отдельной реализации нет, он работает как
  `READ COMMITTED`.
