...
```sql
-- DETACH CONCURRENTLY (PG 14+): без ACCESS EXCLUSIVE на родителе, не блокирует запросы
...
-- ATTACH берёт SHARE UPDATE EXCLUSIVE на родителе (с PG12 — НЕ ACCESS EXCLUSIVE),
...
- `DETACH PARTITION ... CONCURRENTLY` (PG14); у ATTACH `CONCURRENTLY` нет — скан пропускается заранее навешенным совпадающим `CHECK`.
