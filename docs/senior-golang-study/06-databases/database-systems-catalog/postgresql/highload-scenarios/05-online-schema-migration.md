...
- на **PG < 11** любой `DEFAULT` при `ADD COLUMN` **переписывает всю таблицу** под
  `ACCESS EXCLUSIVE` — на 50M строк это минуты-часы полной блокировки (даже `SELECT`
  не пройдёт);
- на **PG 11+** мгновенным будет только **константный** дефолт; **волатильный**
  (`now()`, `gen_random_uuid()`, `nextval`, вычисляемый на строку) всё равно
...
| Что делаем | PG 11+ | PG < 11 |
| --- | --- | --- |
| `ADD COLUMN col int` (без default, nullable) | мгновенно (каталог) | мгновенно |
| `ADD COLUMN col int DEFAULT 0` (константа) | **мгновенно** (missing value) | переписывает таблицу |
| `ADD COLUMN col int NOT NULL DEFAULT 0` (константа) | **мгновенно** | переписывает таблицу |
| `ADD COLUMN id uuid DEFAULT gen_random_uuid()` (волатильно) | переписывает таблицу | переписывает таблицу |
| `ADD COLUMN ts timestamptz DEFAULT now()` (волатильно) | переписывает таблицу | переписывает таблицу |
| `ADD COLUMN x int GENERATED ALWAYS AS (...) STORED` | переписывает таблицу | — |

Вывод: **константное значение на PG 11+ — это безопасно и мгновенно даже на 100M строк.**
Проблема только с per-row значением (или старой версией PG).

---

## Почему константный DEFAULT мгновенный

На PG 11+ `ADD COLUMN ... DEFAULT <константа>` не трогает строки: значение
...
Когда значение нужно вычислять на строку (или проект на PG < 11, или нужен `NOT NULL` с
per-row значением) — разбиваем на шаги, ни один из которых не держит долгий лок:
...
Обход (PG 12+): сначала добавить `CHECK ... NOT VALID`, провалидировать под слабым
...
graph TD
    ...
    B -->|"да, константа"| C{"PG 11+?"}
    C -->|да| D["ADD COLUMN ... DEFAULT const<br/>мгновенно + lock_timeout"]
    C -->|нет| E["как волатильное →<br/>nullable + бэкфилл"]
...
- **Волатильный дефолт переписывает таблицу и на PG 11+** — `now()`,
...
- **`SET NOT NULL` сканирует всю таблицу** под `ACCESS EXCLUSIVE` — обходить через
  `CHECK ... NOT VALID` → `VALIDATE` → `SET NOT NULL` (PG 12+).
...
- PG 11+ и **константный** дефолт: значение кладётся в каталог (`attmissingval`), строки
