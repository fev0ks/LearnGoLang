...
**Зачем.** Операторы `->`, `@>` хороши для простых проверок, но не умеют условий внутри документа («есть ли тег с `priority > 5`», «любой элемент массива дороже 100»). Для этого есть стандартный язык **SQL/JSON path** (тип `jsonpath`) — аналог XPath для JSON.
...
- Язык SQL/JSON path (`jsonpath`) с фильтрами `?(...)`, `like_regex`, диапазонами; операторы `@?` (есть ли матч) и `@@` (предикат истинен), оба под GIN.
