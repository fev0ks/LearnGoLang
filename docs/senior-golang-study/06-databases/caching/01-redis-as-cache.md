...
- [Горячий ключ (hot key)](#горячий-ключ-hot-key)
...
**Базовая аналогия.** БД — это библиотека: каждый запрос — пройти к стеллажу, найти книгу, выписать. Кэш — это стол рядом: книги, которые часто читают, лежат под рукой. Сначала взгляд на стол, и только если там нет — поход в библиотеку.
...
**Не использовать одинаковый TTL для всего.** Если 10000 ключей с TTL=300s закэшировались примерно одновременно — они истекут одновременно, и приложение получит лавину запросов в БД (см. cache stampede).
...
Если несколько инстансов сервиса и каждый держит локальный in-memory кэш (быстрее Redis) — Redis pub/sub для рассылки "invalidate":
...
Cache stampede — это про **истечение** ключа (протух → лавина в БД). Горячий ключ — другая проблема: ключ **в кэше и не истёк**, но обращений к нему столько, что одна нода/шард Redis, где он лежит, перегревается. Шардирование тут не спасает: все запросы по `listing:42` хешируются в один слот → одна нода держит весь трафик ключа.

**Когда возникает:** вирусное объявление, товар с главной страницы, твит знаменитости, «горячий» feature-flag — один ключ даёт заметную долю всего трафика Redis.

```text
Stampede:  ключ истёк  → 1000 запросов идут мимо кэша в БД
Hot key:   ключ в кэше → 1000 запросов идут в ОДНУ ноду Redis → она CPU-bound
```

### Слои защиты (от дешёвого к дорогому)

**1. Локальный in-process кэш (самый эффективный).** Перед Redis — маленький LRU в памяти процесса с TTL в секунды. Большинство попаданий по горячему ключу даже не доходят до Redis (двухуровневый кэш L1→L2).

```go
// L1 (local LRU) → L2 (Redis) → DB
func (c *Cache) Get(ctx context.Context, id string) (*Listing, error) {
    if v, ok := c.local.Get(id); ok {              // L1: in-process, ~0 latency
        return v, nil
    }
    if raw, err := c.redis.Get(ctx, "listing:"+id).Bytes(); err == nil {
        v := decode(raw)
        c.local.Add(id, v, 2*time.Second)          // короткий TTL — допускаем лёгкий стейл
        return v, nil
    }
    // ... miss → DB → заполнить L2 (и L1)
}
```

Цена: до `localTTL` разные инстансы могут отдавать слегка устаревшее значение. Для карточки/профиля приемлемо; для баланса — нет.

**2. Client-side caching (Redis 6+, RESP3 tracking).** Redis сам уведомляет клиента об инвалидации ключа — локальный кэш можно держать дольше без риска отдать стейл. `go-redis` умеет через client tracking; снимает ручную возню с TTL/pub-sub для L1.

**3. Read-реплики.** Горячие чтения раскидать по репликам (Redis replication): запись в мастер, чтение round-robin по репликам. Масштабирует чтение одного ключа линейно числу реплик.

**4. Key splitting (в экстриме).** Если даже реплик мало — размножить значение по N ключам на разных слотах и читать случайный:

```go
shard := rand.Intn(N)
key := fmt.Sprintf("listing:%s:%d", id, shard)  // N копий на разных нодах
```

Размазывает один горячий ключ по N нодам. Цена — при инвалидации удалить все N копий (`listing:42:0..N`).

### Структура значения: String или Hash

| Хранить как | Когда | Почему / минус |
|---|---|---|
| **String** (один blob) | значение читается целиком | один `GET`, атомарная замена `SET`, простой TTL — выбор для read-heavy объекта |
| **Hash** (поля) | частые точечные апдейты отдельных полей | `HGET`/`HSET` по полю, но нет TTL на поле и оверхед памяти на мелочи |

Для карточки/профиля (читаем целиком, обновляем редко и целиком) — **String**.

### Hash-tags в Redis Cluster

`MGET listing:1 listing:2` в кластере может задеть разные слоты — клиент сам разнесёт запрос по нодам, это нормально и даже желательно (параллелизм). Принудительно класть ключи в один слот через hash-tag `{...}` нужно только для атомарных мультиключевых операций (Lua, транзакции). Для независимых карточек hash-tag, наоборот, собрал бы их в один горячий слот — не делать.
...
Если БД-запрос 5 секунд и его результат кладётся в кэш — да, последующие быстрые. Но первый запрос (cache miss) всё равно 5 секунд. Решение — оптимизировать запрос или денормализовать. Кэш не лечит структурные проблемы.
