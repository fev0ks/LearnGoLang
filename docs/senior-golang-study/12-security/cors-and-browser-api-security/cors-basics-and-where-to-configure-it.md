...
Коротко:
- `CORS` не защищает backend от любого внешнего traffic;
- `CORS` управляет тем, что браузер разрешит JavaScript-коду читать при cross-origin запросе;
- это часть browser security model.
